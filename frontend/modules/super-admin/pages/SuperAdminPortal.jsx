...
import SalesTargetManagementView from '../components/targets/SalesTargetManagementView';
import ProductionTargetManagementView from '../components/targets/ProductionTargetManagementView';
...
import {
  Users, User, Send, UserCheck, ShieldAlert, FileText, Layers, BarChart3, LayoutGrid,
...
export default function SuperAdminPortal({ initialView, forceView } = {}) {
  const pathname = usePathname();
  const params = useParams();
  const navigate = useRouter();
  const pathSlug = pathname ? pathname.split('/').filter(Boolean) : [];
  const view = forceView || params?.slug?.[0] || initialView || (pathSlug.length > 1 ? pathSlug[1] : 'dashboard') || 'dashboard';
  ...
  const renderSalesTarget = () => {
    return (
      <SalesTargetManagementView
        salesTargets={salesTargets}
        setSalesTargets={setSalesTargets}
        orders={allOrdersForTargetCalculations}
        usersList={usersList}
        apiClient={apiClient}
        showToast={showToast}
        fireSwal={fireSwal}
  ...
  const renderProductionTarget = () => {
    return (
      <ProductionTargetManagementView
        productionTargets={productionTargets}
        setProductionTargets={setProductionTargets}
        apiClient={apiClient}
        showToast={showToast}
        fireSwal={fireSwal}
        queryClient={queryClient}
