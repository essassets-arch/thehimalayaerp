...
import {
  ...
  TrendingUp, Activity, Settings, Info, Save, Mail, MessageSquare, Briefcase, Eye, Box, Package, Plus, FlaskConical, Tag,
  Download, Search, ChevronLeft, ChevronRight, RefreshCw, Bell, ClipboardList, Trash, ArrowRight, Check, X, Wrench, Truck, Upload,
  Building, MapPin, ShoppingCart, FileCheck, Clock, Camera
} from 'lucide-react';
...
export default function SuperAdminPortal() {
  ...
  // Attendance Monitor States
  const [selectedStaffSim, setSelectedStaffSim] = useState('EMP-002');
    ...
    try {
      ...
      if (data.success || Array.isArray(data.data)) {
        ...
        setProductCategories(prev => {
          const combined = Array.from(new Set([...dbCategories, ...prev]));
          return combined.length > 0 ? combined : dbCategories;
        });
      }
    } catch (e) {
      console.error('Failed to load real backend categories:', e);
    }
  }, []);
  ...
  const renderCategories = () => {
    return <CategoryMasterUI role={currentUser?.role || 'Super Admin'} />;
  };
  ...
  const renderAttendanceWorkspace = () => {
    ...
    const simDate = '2026-06-11';

    const handleExportCSV = (data) => {
      const csvContent = "data:text/csv;charset=utf-8," 
        + data.map(l => `${l.name},${l.id},${l.punchIn},${l.punchOut},${l.status},${l.location || 'N/A'}`).join('\n');
      const encodedUri = encodeURI(csvContent);
      const link = document.createElement("a");
      link.setAttribute("href", encodedUri);
      link.setAttribute("download", "attendance-audit-logs.csv");
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: '20px', alignItems: 'stretch' }}>
            ...
            <div className="app-card" style={{ padding: '20px', background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '14px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <button onClick={() => handleExportCSV(formattedLogs)} style={{ background: '#f8fafc', border: '1px solid #cbd5e1', padding: '6px 12px', borderRadius: '6px', fontSize: '11.5px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px', color: '#475569' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div className="app-card" style={{ padding: '20px', background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '14px', flex: 1, display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              {/* Details inspection description */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', background: '#F8FAFC', padding: '14px', borderRadius: '10px', border: '1px solid #E2E8F0' }}>
                <div>
                  <span style={{ fontSize: '10px', color: '#64748B', display: 'block', textTransform: 'uppercase', fontWeight: '800' }}>Employee Name:</span>
                  <span style={{ fontSize: '12.5px', fontWeight: '700', color: '#0F172A' }}>{activePreview ? activePreview.name : 'Not Audited'} {activePreview ? `(${activePreview.id})` : ''}</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  <div>
                    <span style={{ fontSize: '10px', color: '#64748B', display: 'block', textTransform: 'uppercase', fontWeight: '800' }}>Punch Action:</span>
                    <span style={{ fontSize: '12.5px', fontWeight: '700', color: '#0F172A' }}>{activePreview ? activePreview.action : 'N/A'}</span>
                  </div>
                  <div>
                    <span style={{ fontSize: '10px', color: '#64748B', display: 'block', textTransform: 'uppercase', fontWeight: '800' }}>Device Timestamp:</span>
                    <span style={{ fontSize: '12.5px', fontWeight: '700', color: '#0F172A' }}>{activePreview ? activePreview.time : 'N/A'} {activePreview?.date ? `(${activePreview.date})` : ''}</span>
                  </div>
                </div>
                <div>
                  <span style={{ fontSize: '10px', color: '#64748B', display: 'block', textTransform: 'uppercase', fontWeight: '800' }}>Verified Geolocation Address:</span>
                  <span style={{ fontSize: '12px', fontWeight: '600', color: '#334155', display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <MapPin size={12} color="#0EA5E9" /> {activePreview ? activePreview.location : 'N/A'}
                  </span>
                </div>
              </div>
