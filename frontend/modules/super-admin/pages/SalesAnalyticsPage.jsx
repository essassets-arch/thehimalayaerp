...
const SalesAnalyticsPage = () => {
  ...
  // Custom BI Hooks
  const { filters, setFilters, savedViews, saveCurrentView, deleteSavedView, applyPreset } = useSalesFilters();
  const { data, loading, refreshAll } = useCommandCenter(filters);
  ...
  // 1. Calculations & Metrics fallbacks
  const overviewData = data.overview || { kpis: [] };
  const exceptionsData = data.exceptions || { exceptions: [] };
  const crmData = data.crm || { metrics: {}, splits: { sources: [], lostReasons: [] } };
  const productionData = data.production || { metrics: {}, efficiency: {} };
  const dispatchData = data.dispatch || {};
  const financeData = data.finance || { billing: 0, outstanding: 0, collected: 0, agingBuckets: {} };
  const employeesData = data.employees || { performance: [] };
  const explorerData = data.explorer || { rows: [] };

  const trendsData = data.trends || [];
  const healthData = data.health || [];

  const revenueTrends = trendsData.length > 0 ? trendsData : [
    { month: 'Jan', revenue: 0 },
    { month: 'Feb', revenue: 0 },
    { month: 'Mar', revenue: 0 },
    { month: 'Apr', revenue: 0 },
  ...
  const filteredExplorerRows = useMemo(() => {
    if (!globalSearch) return explorerData.rows || [];
    const q = globalSearch.toLowerCase();
    return (explorerData.rows || []).filter(row => 
      Object.values(row).some(val => String(val).toLowerCase().includes(q))
    );
  }, [explorerData.rows, globalSearch]);
        ...
        <div style={{ background: '#fff', border: '1px solid #D6E2F0', borderRadius: '12px', padding: '18px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {(healthData.length > 0 ? healthData : [
              { name: 'Sales Pipeline', rating: 0, status: '🔴', color: '#ef4444' },
              { name: 'Production Runtimes', rating: 0, status: '🔴', color: '#ef4444' },
              { name: 'QC Yields', rating: 0, status: '🔴', color: '#ef4444' },
              { name: 'Dispatch & Logistics', rating: 0, status: '🔴', color: '#ef4444' },
              { name: 'Collections Efficiency', rating: 0, status: '🔴', color: '#ef4444' },
              { name: 'Finance / Cash Flows', rating: 0, status: '🔴', color: '#ef4444' }
            ]).map((idxItem, idx) => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '20px' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #DCE5F0', borderRadius: '12px', padding: '18px' }}>
          ...
          <div style={{ height: '200px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={crmData.splits?.sources || []} cx="50%" cy="50%" innerRadius={45} outerRadius={65} paddingAngle={4} dataKey="count" nameKey="source">
                  {crmData.splits?.sources?.map((entry, index) => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(480px, 1fr))', gap: '20px' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #D6E2F0', borderRadius: '12px', padding: '18px' }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
              ...
              <tbody>
                {employeesData.performance?.map((ex, idx) => {
                  const target = 25000000; // Target is ₹2.5 Cr
        ...
        <div style={{ background: '#fff', border: '1px solid #D6E2F0', borderRadius: '12px', padding: '18px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {(() => {
              const buckets = Object.keys(financeData.agingBuckets || {});
              const totalOutstanding = buckets.reduce((sum, b) => sum + parseFloat(financeData.agingBuckets[b] || 0), 0) || 1;
              return buckets.map((bucket, idx) => {
                const val = parseFloat(financeData.agingBuckets[bucket] || 0);
