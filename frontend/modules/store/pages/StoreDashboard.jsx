...
export const StoreDashboard = () => {
  ...
  const [consumptionTrendBackend, setConsumptionTrendBackend] = useState(null);
  const [stockMovementBackend, setStockMovementBackend] = useState([]);
  const [abcAnalysisBackend, setAbcAnalysisBackend] = useState([]);
  const [fsnAnalysisBackend, setFsnAnalysisBackend] = useState([]);
  const [stockAgingBackend, setStockAgingBackend] = useState([]);
  const [topConsumedBackend, setTopConsumedBackend] = useState([]);
  const [attentionMetricsBackend, setAttentionMetricsBackend] = useState(null);
  const [consumptionTableBackend, setConsumptionTableBackend] = useState([]);
    ...
    try {
      ...
      if (dashRes.status === 'fulfilled' && dashRes.value) {
        ...
        const invList = Array.isArray(payload.inventory) ? payload.inventory.map(item => {
          ...
          const itemVal = (available + reserved) * price;
        ...
        if (payload.consumptionTrend) setConsumptionTrendBackend(payload.consumptionTrend);
        if (Array.isArray(payload.stockMovement)) setStockMovementBackend(payload.stockMovement);
        if (Array.isArray(payload.abcAnalysis)) setAbcAnalysisBackend(payload.abcAnalysis);
        if (Array.isArray(payload.fsnAnalysis)) setFsnAnalysisBackend(payload.fsnAnalysis);
        if (Array.isArray(payload.stockAging)) setStockAgingBackend(payload.stockAging);
        if (Array.isArray(payload.topConsumed)) setTopConsumedBackend(payload.topConsumed);
        if (payload.attentionMetrics) setAttentionMetricsBackend(payload.attentionMetrics);
        if (Array.isArray(payload.consumptionTable)) setConsumptionTableBackend(payload.consumptionTable);
      } else {
        setLiveInventory([]);
        setSummaryData(null);
        setConsumptionTrendBackend(null);
        setStockMovementBackend([]);
        setAbcAnalysisBackend([]);
        setFsnAnalysisBackend([]);
        setStockAgingBackend([]);
        setTopConsumedBackend([]);
        setAttentionMetricsBackend(null);
        setConsumptionTableBackend([]);
      }
  ...
  const kpiData = useMemo(() => {
    ...
    const rawMatCount = summaryData?.totalRawMaterials ?? (liveInventory.length > 0 ? liveInventory.length : rawMaterialsList.length);
    ...
    const calculatedIssuedQty = Math.max(txIssuedQty, reqIssuedQty);
    const calculatedMaterialsCount = Math.max(txIssuedMaterials.size, reqIssuedMaterials.size);
    ...
    return {
      totalVal: summaryData?.totalValuation ?? totalVal,
  ...
  const dynamicMaterialOptions = useMemo(() => {
    ...
    if (opts.size === 0 && rawMaterialsList.length > 0) {
      rawMaterialsList.forEach(m => {
        const code = m.sku || m.code;
        const name = m.name;
        if (code && name) {
          opts.set(code, `${name} (${code})`);
        }
      });
    }

    return Array.from(opts.entries()).map(([code, label]) => ({ code, label }));
  }, [liveInventory, rawMaterialsList]);
  ...
  const consumptionChartData = useMemo(() => {
    ...
    // If default "ALL" filter and backend provided trend for this range, use it
    if (
      selectedMaterialFilter === 'ALL' &&
      selectedProductionLine === 'ALL' &&
      selectedDepartment === 'ALL' &&
      consumptionTrendBackend &&
      Array.isArray(consumptionTrendBackend[consumptionTimeRange]) &&
      consumptionTrendBackend[consumptionTimeRange].length > 0
    ) {
      return consumptionTrendBackend[consumptionTimeRange];
    }

    // Filter transactions or material requests strictly dynamically
    const outTransactions = stockTransactions.filter(tx => {
      const txType = (tx.type || '').toUpperCase();
      const isOut = ['OUT', 'ISSUE', 'PRODUCTION_ISSUE', 'STOCK_OUT'].includes(txType) || tx.referenceType === 'ISSUE_TO_PRODUCTION';
      if (!isOut) return false;
      if (selectedMaterialFilter !== 'ALL') {
        const matMatch = `${tx.productId || ''} ${tx.rawMaterialId || ''} ${tx.product?.sku || ''} ${tx.product?.name || ''} ${tx.materialName || ''}`.toLowerCase();
    ...
    outTransactions.forEach(tx => {
      ...
      if (periodMap[key] !== undefined) {
        periodMap[key] += Math.abs(Number(tx.quantity) || 0);
      }
    });

    return periods.map(period => {
      const qty = periodMap[period] || 0;
      return {
        period,
        IssuedQuantity: qty,
        TargetConsumption: Math.round(qty * 1.1)
      };
    });
  }, [stockTransactions, consumptionTimeRange, selectedMaterialFilter, selectedProductionLine, selectedDepartment, consumptionTrendBackend]);

  // ── 2. Stock Movement Data (Dynamic Inward vs Consumption) ──
  const stockMovementData = useMemo(() => {
    if (Array.isArray(stockMovementBackend) && stockMovementBackend.length > 0) {
      return stockMovementBackend;
    }
  ...
  }, [stockTransactions, stockMovementBackend]);

  // ── 3. Inventory ABC Analysis Donut Data (Dynamic Math) ──
  const abcDonutData = useMemo(() => {
    if (Array.isArray(abcAnalysisBackend) && abcAnalysisBackend.length > 0) {
      return abcAnalysisBackend;
    }
    let classA = 0, classB = 0, classC = 0;
    if (liveInventory.length > 0) {
      const itemsWithValue = liveInventory.map(item => ({
        ...item,
        totalVal: (Number(item.available || 0) + Number(item.reserved || 0)) * Number(item.price || 0)
      ...
      if (grandVal > 0) {
        return [
          { name: `A — High-Value (${Math.round((classA / grandVal) * 100)}%)`, value: Number((classA / 100000).toFixed(2)), color: '#0284c7' },
          { name: `B — Medium-Value (${Math.round((classB / grandVal) * 100)}%)`, value: Number((classB / 100000).toFixed(2)), color: '#f59e0b' },
          { name: `C — Low-Value (${Math.round((classC / grandVal) * 100)}%)`, value: Number((classC / 100000).toFixed(2)), color: '#64748b' }
        ];
      }
    }
    return [
      { name: 'A — High-Value (0%)', value: 0, color: '#0284c7' },
      { name: 'B — Medium-Value (0%)', value: 0, color: '#f59e0b' },
      { name: 'C — Low-Value (0%)', value: 0, color: '#64748b' }
    ];
  }, [liveInventory, abcAnalysisBackend]);

  // ── 4. FSN Analysis Donut Data (Dynamic Movement Velocity) ──
  const fsnDonutData = useMemo(() => {
    if (Array.isArray(fsnAnalysisBackend) && fsnAnalysisBackend.length > 0) {
      return fsnAnalysisBackend;
    }
  ...
  }, [liveInventory, fsnAnalysisBackend]);

  // ── 5. Stock Aging Stacked Bar Data (Dynamic Buckets) ──
  const stockAgingData = useMemo(() => {
    if (Array.isArray(stockAgingBackend) && stockAgingBackend.length > 0) {
      return stockAgingBackend;
    }
    ...
    return [
      { bucket: '0–30 Days', percent: total > 0 ? Math.round((b0_30 / total) * 100) : 0, skus: b0_30, color: '#10b981' },
      { bucket: '31–90 Days', percent: total > 0 ? Math.round((b31_90 / total) * 100) : 0, skus: b31_90, color: '#0284c7' },
      { bucket: '91–180 Days', percent: total > 0 ? Math.round((b91_180 / total) * 100) : 0, skus: b91_180, color: '#f59e0b' },
      { bucket: '>180 Days', percent: total > 0 ? Math.round((b180Plus / total) * 100) : 0, skus: b180Plus, color: '#ef4444' }
    ];
  }, [liveInventory, stockAgingBackend]);

  // ── 6. Top 10 Materials Consumed Data (Dynamic Consumption Ranking) ──
  const topMaterialsConsumed = useMemo(() => {
    if (Array.isArray(topConsumedBackend) && topConsumedBackend.length > 0) {
      return topConsumedBackend;
    }
    if (liveInventory.length > 0) {
      const txMap = new Map();
      stockTransactions.forEach(tx => {
    ...
    return [];
  }, [liveInventory, stockTransactions, topConsumedBackend]);

  // ── 7. Production Material Consumption Details Table Data (Dynamic Accounting) ──
  const consumptionTableData = useMemo(() => {
    if (Array.isArray(consumptionTableBackend) && consumptionTableBackend.length > 0) {
      return consumptionTableBackend;
    }
    if (liveInventory.length > 0) {
      const txMap = new Map();
      stockTransactions.forEach(tx => {
      ...
      return liveInventory.slice(0, 15).map((inv) => {
        const matKey = (inv.id || inv.code || inv.name || '').toString().toLowerCase();
        const codeKey = (inv.code || '').toString().toLowerCase();
        const txData = txMap.get(matKey) || txMap.get(codeKey) || { received: 0, issued: 0, returnQty: 0 };
    ...
    return [];
  }, [liveInventory, stockTransactions, consumptionTableBackend]);


  // ── 8. Attention Required Panel Metrics (Dynamic Operational Counts) ──
  const attentionPanelMetrics = useMemo(() => {
    if (attentionMetricsBackend) {
      return attentionMetricsBackend;
    }
    let belowMin = 0;
    let nearMin = 0;
    let nearMax = 0;
  ...
  }, [liveInventory, qcInspectionsList, materialRequestsList, procurementIndentsList, attentionMetricsBackend]);
    ...
    <div className="store-dashboard-wrapper m-theme-container">
      ...
      <div className="store-chart-card" style={{ marginBottom: '24px' }}>
        ...
        {topMaterialsConsumed.length > 0 ? (
          ...
          <div style={{ padding: '36px 16px', textAlign: 'center', color: '#64748b', fontSize: '13px', fontWeight: '600' }}>
            No material consumption records found in database.
          </div>
        )}
      </div>
      ...
      <div className="store-chart-card">
          ...
          <table style={{ width: '100%', minWidth: '780px', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
            ...
            <tbody>
              {consumptionTableData.length > 0 ? (
                ...
                <tr>
                  <td colSpan={8} style={{ padding: '32px 16px', textAlign: 'center', color: '#64748b', fontWeight: '600', fontSize: '13px' }}>
                    No material consumption records found in database.
                  </td>
                </tr>
              )}
        ...
        <div className="mobile-only raw-inventory-mobile-list" style={{ gap: '10px' }}>
          {consumptionTableData.length > 0 ? (
            ...
            <div style={{ padding: '24px 16px', textAlign: 'center', color: '#64748b', background: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', fontSize: '13px', fontWeight: '600' }}>
              No material consumption records found in database.
            </div>
          )}
        </div>
            ...
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '16px', border: '1px solid #e2e8f0' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '13px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px stroke #e2e8f0', paddingBottom: '6px' }}>
                  ...
                  <span style={{ fontWeight: '800', color: '#0f172a' }}>{selectedMaterialDetail.warehouse || 'Main Store Depot'}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px stroke #e2e8f0', paddingBottom: '6px' }}>
