...
export default function FinancePaymentHistoryPage() {
      ...
      <div className="erp-header-card">
        <div className="erp-header-title-group">
          <h2 className="erp-header-title">
            <History style={{ width: 24, height: 24, color: '#4f46e5' }} />
            Finance → Verified Payment History Archive
          </h2>
          <p className="erp-header-subtitle">
            Official ledger of all verified payments, partial collections, and settled accounts.
          </p>
        </div>
              ...
              {filteredLedgers.length === 0 ? (
                <tr>
                  <td colSpan={8} style={{ padding: '32px', textAlign: 'center', color: '#8893A7', fontStyle: 'italic' }}>
                    No verified payment entries found.
                  </td>
                </tr>
              ) : (
                filteredLedgers.map((tx: any, idx: number) => (
                  <tr key={tx.id || idx}>
                    ...
                    <td style={{ color: '#475569' }}>{tx.verifiedBy || 'Finance Manager'}</td>
                    <td style={{ textAlign: 'right' }}>
                      <StandardActionButtons
                        onDownload={() => alert(`Downloading voucher for UTR ${tx.transactionId}...`)}
                        onShare={() => alert(`Sharing payment voucher...`)}
