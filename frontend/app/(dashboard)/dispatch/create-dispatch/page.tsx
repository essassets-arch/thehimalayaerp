...
import { Truck, ArrowLeft, Send, X, ClipboardList, Info } from "lucide-react";
...
export default function CreateDispatchPage() {
  ...
  const {
    ...
    queryKey: ["work-order-detail-create", workOrderId],
    queryFn: async () => {
      if (!workOrderId) throw new Error("Work Order ID missing");
      return backendFetch<WorkOrder>(
        `/api/backend/production/work-orders/${workOrderId}`,
      );
    },
    enabled: !!workOrderId,
  });
  ...
  const handleSubmit = async () => {
    ...
    if (dispatchQty > approvedQty) {
      toast.error(
        `Dispatch Quantity cannot exceed QC-approved quantity (${approvedQty})`,
      );
      return;
    }
    if (dispatchQty > remainingOrderQty) {
      toast.error(
        `Dispatch Quantity cannot exceed remaining order quantity (${remainingOrderQty})`,
      );
      return;
    }
    ...
    try {
      ...
      // Create Dispatch (which is saved as IN_TRANSIT directly)
      await backendFetch<unknown>("/api/backend/logistics/dispatches", {
        method: "POST",
        body: payload,
      });

      toast.success("Dispatch created and marked In Transit");
    ...
    return (
      <div className="max-w-2xl mx-auto mt-12 p-6 bg-red-50 border border-red-200 rounded-xl">
        ...
        <p className="text-sm text-red-700 mt-1">
          Make sure a valid Work Order ID is provided.
        </p>
        <Button
      ...
      <div className={styles.card}>
        ...
        <div className={styles.cardHeader}>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Fill out the details to initiate dispatch for Sales Order{" "}
        ...
        <div className={styles.notice}>
          ...
          <div className="space-y-1">
            ...
            <div className="flex flex-wrap gap-4 text-xs text-blue-800 mt-1 font-mono">
              <div>
                Ordered Qty:{" "}
                <span className="font-bold text-gray-900">{orderedQty}</span>
              </div>
              <div>
                Already Dispatched:{" "}
          ...
          <div className={styles.column}>
            ...
            <div>
              ...
              <input
                ...
                disabled
                className="w-full bg-gray-50 text-gray-400 border rounded-lg px-3 py-2 text-sm font-medium"
              />
            </div>
          ...
          <div className={styles.column}>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                placeholder="Gate entry details, timing restrictions, etc."
              />
            </div>
          </div>
        </div>
