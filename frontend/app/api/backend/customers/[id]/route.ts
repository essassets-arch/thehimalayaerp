...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const safeId = encodeURIComponent(id);
  const policy = bridgePolicies.updateCustomer;
  const idempotencyKey = request.headers.get('idempotency-key');

  if (policy.requireIdempotencyKey && !isValidUuid(idempotencyKey)) {
    return new Response(
      JSON.stringify({ success: false, message: 'A valid UUID Idempotency-Key header is required.', code: 'BAD_REQUEST' }),
