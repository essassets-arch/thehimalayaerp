...
export async function POST(request: Request) {
  try {
    ...
    // Forward the incoming Cookie header (which contains refreshToken) to NestJS
    ...
    const json = await res.json().catch(() => ({ success: false, message: 'Invalid response from backend' }));
