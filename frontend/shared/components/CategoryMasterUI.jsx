    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div className="erp-kpi-grid">
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: '14px', padding: isMobile ? '14px 16px' : '20px 22px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative', overflow: 'hidden', minWidth: 0 }}>
          ...
          <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(16, 185, 129, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#10B981', flexShrink: 0 }}>
            <Package size={20} />
          </div>
        </div>
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: '14px', padding: isMobile ? '14px 16px' : '20px 22px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative', overflow: 'hidden', minWidth: 0 }}>
          ...
          <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(245, 158, 11, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#D97706', flexShrink: 0 }}>
            <Info size={20} />
          </div>
        </div>
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: '14px', padding: isMobile ? '16px' : '24px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: isMobile ? 'stretch' : 'center', marginBottom: '20px', flexDirection: isMobile ? 'column' : 'row', gap: '14px' }}>
          <div>
            <h2 style={{ fontSize: isMobile ? '18px' : '20px', fontWeight: 700, color: '#0F172A', margin: 0 }}>Product Catalog Categories</h2>
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center', width: isMobile ? '100%' : 'auto' }}>
            <button
              ...
              style={{ flex: isMobile ? 1 : 'none', justifyContent: 'center', background: '#FFFFFF', border: '1px solid #E2E8F0', padding: '9px 16px', borderRadius: '8px', color: '#334155', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', fontSize: '13px' }}
            >
              <RefreshCw size={15} className={loading ? 'spin' : ''} /> Refresh
            </button>
            <button
              style={{ flex: isMobile ? 1 : 'none', justifyContent: 'center', background: 'linear-gradient(135deg, #6366F1 0%, #4F46E5 100%)', border: 'none', padding: '9px 18px', borderRadius: '8px', color: '#FFFFFF', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', fontSize: '13px', boxShadow: '0 4px 12px rgba(79, 70, 229, 0.25)' }}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {productCategories.length === 0 ? (
              ...
              productCategories.map((cat) => {
                const matching = realBackendProducts.filter(p => p.category === cat || p.product_family === cat);
                ...
                const previewText = count === 0 ? 'No items mapped' : `${samples}${extra}`;
                ...
                return (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1, minWidth: 0 }}>
                      <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(99, 102, 241, 0.1)', color: '#6366F1', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <Tag size={18} />
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', minWidth: 0, flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                          <span style={{ fontWeight: 800, color: '#0F172A', fontSize: '15px' }}>{cat}</span>
                          <span style={{ fontSize: '11px', fontWeight: 700, padding: '2px 8px', borderRadius: '12px', background: '#EEF2FF', color: '#4338CA', border: '1px solid #C7D2FE', whiteSpace: 'nowrap' }}>
                            {count} Products
                          </span>
                        </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                      {count > 0 ? (
                        <span 
                          ...
                          title={`Cannot delete: ${count} active catalog products are mapped to ${cat}`}
                        >
