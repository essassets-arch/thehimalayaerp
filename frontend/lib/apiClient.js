...
function shouldProxyToBackend(path) {
  const result = path.includes('/payroll') || path.includes('/salary-slips') || path.includes('/purchase/vendors') || path.includes('/suppliers') || path.includes('/vendors') || path.includes('/complaints') || path.includes('/back-office') || path.includes('/backoffice') || path.includes('/attendance') || path.includes('/products') || path.includes('/finance') || path.includes('/profile') || path.includes('/expenses') || path.includes('/leaves') || path.includes('/attendance-requests') || path.includes('/notifications') || path.includes('/sales/samples') || path.includes('/crm/leads') || path.includes('/logistics/dispatches') || path.includes('/plant-head/dashboard-data') || path.includes('/plant-head/analytics/production') || path.includes('/plant-head/analytics/material') || path.includes('/plant-head/overview/departments') || path.includes('/plant-head/reports/generate-ai') || path.includes('/plant-head/incoming-orders') || path.includes('/plant-head/planning-orders') || path.includes('/plant-head/planning') || path.includes('/replacements') || path.includes('/sales-returns') || path.includes('/inventory') || path.includes('/qc') || path.includes('/production/finished-goods') || path.includes('/material-requests') || path.includes('/plant-head/material-indents') || path.includes('/hr/') || path.includes('/store-reports') || path.includes('/admin/') || path.includes('/backend/') || path.includes('/brand-analysis') || path.includes('/customer-complaints') || path.includes('/procurement/') || path.includes('/reports/') || path.includes('/sales/orders');
  return result;
}
    ...
    if (!res.ok) {
      ...
      if (data && typeof data === 'object') {
        if (data.error && typeof data.error === 'object' && data.error.message) {
          ...
          errMsg = Array.isArray(data.message) ? data.message[0] : data.message;
