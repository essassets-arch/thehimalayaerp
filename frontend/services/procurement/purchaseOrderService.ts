...
export const purchaseOrderService = {
  get: (id: string) => procurementRequest(`purchase-orders/${id}`, 'GET', undefined, { cacheTtlMs: 0 }),
  list: (query?: Record<string, any>) => procurementRequest('purchase-orders', 'GET', undefined, { query, cacheTtlMs: 0 }),
  ...
  createFromIndent: (indentId: string, data: unknown) => {
    const isLocal = !indentId || indentId.startsWith('DEMO-') || indentId.startsWith('LOCAL-');
    if (isLocal) {
  ...
  createFromSelectedIndents: (data: unknown) => {
    return procurementRequest('purchase-orders/from-selected-indents', 'POST', data);
  },
