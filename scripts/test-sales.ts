...
// Custom storage for Node.js file system
const fileStorage = {
    getItem: (name: string) => {
        try {
            return fs.readFileSync(path.join(process.cwd(), `${name}.json`), 'utf-8');
        } catch (e) {
            return null;
        }
    },
    setItem: (name: string, value: string) => {
        fs.writeFileSync(path.join(process.cwd(), `${name}.json`), value, 'utf-8');
    },
    removeItem: (name: string) => {
        try {
            fs.unlinkSync(path.join(process.cwd(), `${name}.json`));
        } catch (e) {}
    }
};

// Create a Vanilla Zustand store with persistence
...
async function runSalesTests() {
    console.log("==========================================");
    console.log("🚀 Running Sales Flow Tests with Zustand (Local Storage)...");
    console.log("==========================================\n");
    ...
    try {
        ...
        console.log("🏆 ALL SALES TESTS PASSED SUCCESSFULLY! Data stored locally in local-sales-db.json");
