#!/usr/bin/env bash
# ==============================================================================
# HIMALAYA ERP — SYNC SALES 1 (JP) COMPLETE PIPELINE (LEAD -> ORDER -> PLANT HEAD -> READY FOR DISPATCH)
# ==============================================================================
set -Eeuo pipefail
...
echo "========================================================================"
echo "⚡ SYNCING SALES 1 (JP / sales1@himalayaerp.com) PIPELINE ON VPS / DOCKER"
echo "========================================================================"
...
if [ -n "$CONTAINER_NAME" ]; then
    ...
    docker cp backend/scripts/sync_sales1_jp_proper_aligned.js "$CONTAINER_NAME":/app/scripts/ || true
    docker cp backend/scripts/sync_sales1_jp_proper_aligned.js "$CONTAINER_NAME":/app/ || true
    
    echo "Executing Sales 1 sync script in container..."
    docker exec -i "$CONTAINER_NAME" node scripts/sync_sales1_jp_proper_aligned.js || docker exec -i "$CONTAINER_NAME" node sync_sales1_jp_proper_aligned.js
    ...
    docker compose cp backend/scripts/sync_sales1_jp_proper_aligned.js backend:/app/scripts/ || true
    docker compose cp backend/scripts/sync_sales1_jp_proper_aligned.js backend:/app/ || true
    
    echo "Executing Sales 1 sync via docker compose..."
    docker compose exec backend node scripts/sync_sales1_jp_proper_aligned.js

elif [ -f "/.dockerenv" ] || [ -n "${DATABASE_URL:-}" ]; then
    echo "Running in local / standalone Node environment..."
    node backend/scripts/sync_sales1_jp_proper_aligned.js
else
    echo "Running with local node..."
    if [ -d "backend" ]; then
        (cd backend && node scripts/sync_sales1_jp_proper_aligned.js)
    else
        node sync_sales1_jp_proper_aligned.js
    fi
fi

echo "========================================================================"
echo "✅ Sales 1 (Lead -> Quote -> Order -> Plant Head -> Ready for Dispatch) Completed!"
echo "========================================================================"
