...
echo "========================================================================"
echo "🚀 SYNC SALES 3 (RT) TO READY FOR DISPATCH"
echo "========================================================================"
...
docker cp backend/scripts/sync_sales3_pipeline_to_ready_dispatch.js himalaya-backend:/app/scripts/
docker cp backend/scripts/sales3_leads_data.json himalaya-backend:/app/scripts/
docker cp backend/scripts/ravi_thakor_sales3.csv himalaya-backend:/app/scripts/
...
docker exec himalaya-backend node scripts/sync_sales3_pipeline_to_ready_dispatch.js
...
async function verify() {
  ...
  const s3Orders = await prisma.salesOrder.count({ where: { salesExecutive: { email: 'sales3@himalayaerp.com' } } });
  const fgCount = await prisma.finishedGoods.count({ where: { status: 'AVAILABLE' } });
  console.log('--- VERIFICATION SUMMARY ---');
  console.log('✔ Sales 3 Orders in DB :', s3Orders);
...
echo "========================================================================"
echo "🎉 SUCCESS: All 15 Orders & 22 Work Orders for Sales 3 are READY FOR DISPATCH!"
