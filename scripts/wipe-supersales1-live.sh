...
if docker ps --format '{{.Names}}' | grep -q "^himalaya-backend$"; then
  ...
  docker exec -i himalaya-backend node - < backend/scripts/wipe_all_supersales1_data.js
  ...
  docker compose exec -T backend node - < backend/scripts/wipe_all_supersales1_data.js
elif [ -d "backend" ]; then
