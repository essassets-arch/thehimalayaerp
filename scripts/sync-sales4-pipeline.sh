#!/usr/bin/env bash
# ==============================================================================
# HIMALAYA ERP — IMPORT DATA TO SALES FOUR & CONVERT TO QUOTES & ORDERS
# ==============================================================================
set -Eeuo pipefail
...
echo "======================================================================"
echo "🚀 IMPORTING DATA INTO SALES FOUR & CONVERTING TO QUOTES & ORDERS"
echo "======================================================================"
...
if docker ps --format '{{.Names}}' | grep -q "^himalaya-backend$"; then
  ...
  docker cp backend/scripts/sync_sales4_full_pipeline.js himalaya-backend:/app/scripts/ || true
  
  echo "Running sync inside himalaya-backend container..."
  docker exec -i himalaya-backend node scripts/sync_sales4_full_pipeline.js
  ...
  docker compose cp backend/scripts/sync_sales4_full_pipeline.js backend:/app/scripts/ || true
  
  echo "Running sync via docker compose backend..."
  docker compose exec backend node scripts/sync_sales4_full_pipeline.js

elif [ -d "backend" ]; then
  echo "Running on host backend..."
  (cd backend && node scripts/sync_sales4_full_pipeline.js)
else
  node scripts/sync_sales4_full_pipeline.js
fi

echo ""
echo "======================================================================"
echo "✅ SALES FOUR IMPORT & FULL PIPELINE CONVERSION COMPLETE!"
echo "======================================================================"
