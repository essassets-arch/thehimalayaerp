...
export default function OrdersView({ 
  ...
  const getOrderActionState = (order) => {
    ...
    if (['PRODUCTION_STARTED', 'PRODUCTION_IN_PROGRESS', 'IN_PRODUCTION'].includes(order.productionStatus)) {
                ...
                return (
                  <tr key={o.id || o.orderNo}>
                    ...
                    <td data-label="Products / Items">
                      {o.products || (Array.isArray(o.items) && o.items.length > 0 ? o.items.map(i => `${i.productName || i.name || 'Item'} (${i.quantity || 1} Qty)`).join(', ') : '') || (Array.isArray(o.detailedItems) && o.detailedItems.length > 0 ? o.detailedItems.map(i => `${i.productName || i.name || 'Item'} (${i.quantity || 1} Qty)`).join(', ') : '') || '—'}
                    </td>
                    {!isProductionUser && (
