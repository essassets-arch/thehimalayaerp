...
const CustomTooltip = ({ active, payload }: TooltipProps) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-popover border border-border rounded-lg p-3 shadow-lg" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-soft)', padding: '12px', borderRadius: '8px' }}>
        <div className="text-sm text-muted-foreground mb-1" style={{ color: 'var(--text-secondary)' }}>{data.date}</div>
        <div className="flex items-center gap-2">
          <div className="text-base font-bold" style={{ color: 'var(--text-primary)', fontWeight: 'bold' }}>${(data.value * 10).toLocaleString()}.00</div>
          <div className="text-[11px] text-emerald-600" style={{ color: '#16a34a', fontWeight: 'bold' }}>+12.7%</div>
        </div>
      </div>
    );
  }
