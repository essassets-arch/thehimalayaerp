...
export function ProcurementAuditTimeline({ logs }) {
  ...
  return (
    <div className="flow-root mt-4">
      <ul className="-mb-8">
        {logs.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)).map((log, logIdx) => (
          <li key={log.id}>
                ...
                <div className="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                  <div>
                    <p className="text-sm text-gray-500">
                      <span className="font-medium text-gray-900">{log.actorName} ({log.actorRole})</span>
                  ...
                  <div className="whitespace-nowrap text-right text-sm text-gray-500 flex items-center gap-1">
                    <Clock size={14} />
                    <time dateTime={log.timestamp}>{formatDate(new Date(log.timestamp))}</time>
                  </div>
                </div>
              </div>
            </div>
