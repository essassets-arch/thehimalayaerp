...
const products = require('./live_products.json');
const consolidatedLeads = require('./consolidated_leads_preview.json');
...
for (const lead of consolidatedLeads) {
  for (const it of lead.items) {
    ...
    if (m) {
      ...
      unmatchedCount++;
      unmatchedItems.push(`${it.product} ${it.size} ${it.capacity}`);
    }
  }
}

console.log(`Matched items: ${matchedCount} / ${matchedCount + unmatchedCount}`);
if (unmatchedCount > 0) {
  console.log('Sample unmatched items:', Array.from(new Set(unmatchedItems)).slice(0, 10));
}
