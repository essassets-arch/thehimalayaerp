...
function loadAllSuperSales2Leads() {
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    const siteInchargeMobile = cleanPhone(r[6] || r[7] || '');
    const officeContact = cleanPhone(r[7] || '');
...
console.log(`Successfully prepared ${leads.length} leads.`);
leads.forEach((l, i) => {
  console.log(`${i+1}. [${l.rawDate}] ${l.companyName} | ${l.address.city}, ${l.address.state} | ${l.phone} | Items: ${l.items.length} | Qty: ${l.estimatedQuantity}`);
});
