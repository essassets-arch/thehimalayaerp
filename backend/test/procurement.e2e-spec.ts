...
describe('Procurement — Happy Path (Phase 1–6)', () => {
  ...
  afterAll(async () => {
    ...
    if (invoiceId) {
      await prisma.vendorInvoiceItem.deleteMany({
        where: { vendorInvoiceId: invoiceId },
      });
      await prisma.vendorInvoice.deleteMany({ where: { id: invoiceId } });
    }
    if (grnId) {
      await prisma.gRNStatusHistory.deleteMany({
    ...
    if (indentId) {
      await prisma.purchaseIndentStatusHistory.deleteMany({
  ...
  describe('1 · Purchase Indent', () => {
    it('creates indent in DRAFT status', async () => {
      const res = await api(app)
        ...
        .send({
          ...
          businessReason: 'Happy path E2E',
          items: [{ productId, quantity: 50, estimatedUnitRate: 20 }],
        })
        .expect(201);
    ...
    it('submits indent for Plant Head approval', async () => {
      const res = await api(app)
        .post(`/procurement/indents/${indentId}/submit`)
        .set('Authorization', `Bearer ${storeToken}`)
        .send({})
        .expect(201);
    ...
    it('Plant Head approves indent', async () => {
      const res = await api(app)
        ...
        .send({
          remarks: 'Approved',
          items: [{ productId, approvedQuantity: 50, quantity: 50 }],
        })
        .expect(201);
  ...
  describe('4 · Vendor Invoice', () => {
    it('Finance creates vendor invoice', async () => {
      const res = await api(app)
        ...
        .send({
          ...
          totalAmount: 1180,
          dueDate: new Date(),
          items: [{ productId, quantity: 50, unitRate: 20, gstPercent: 18 }],
        })
        .expect(201);
    ...
    it('submits invoice for matching', async () => {
      const res = await api(app)
        .post(`/procurement/vendor-invoices/${invoiceId}/submit`)
        .set('Authorization', `Bearer ${financeToken}`)
        .send({})
        .expect(201);
    ...
    it('3-way match passes and invoice becomes VERIFIED', async () => {
      const res = await api(app)
        .post(`/procurement/vendor-invoices/${invoiceId}/run-match`)
        .set('Authorization', `Bearer ${financeToken}`)
        .send({})
        .expect(201);
  ...
  describe('5 · Vendor Payment', () => {
    it('Finance records payment', async () => {
      const res = await api(app)
        ...
        .send({
          supplierId,
          paidAmount: 1180,
          allocations: [{ vendorInvoiceId: invoiceId, amount: 1180 }],
        })
        .expect(201);
    ...
    it('submits payment for approval', async () => {
      const res = await api(app)
        .post(`/procurement/vendor-payments/${paymentId}/submit`)
        .set('Authorization', `Bearer ${financeToken}`)
        .send({})
        .expect(201);
    ...
    it('Super Admin approves payment', async () => {
      const res = await api(app)
        .post(`/procurement/vendor-payments/${paymentId}/approve`)
        .set('Authorization', `Bearer ${superToken}`)
        .send({})
        .expect(201);
    ...
    it('moves to PROCESSING', async () => {
      const res = await api(app)
        .post(`/procurement/vendor-payments/${paymentId}/process`)
        .set('Authorization', `Bearer ${financeToken}`)
        .send({})
        .expect(201);
    ...
    it('completes payment — invoice becomes PAID', async () => {
      await api(app)
        .post(`/procurement/vendor-payments/${paymentId}/complete`)
        .set('Authorization', `Bearer ${financeToken}`)
        .send({ transactionId: `TX-HP-${Date.now()}` })
        .expect(201);
  ...
  describe('6 · PO Closure', () => {
    it('closure evaluator reports eligible with 0 blockers', async () => {
      const res = await api(app)
        .get(`/procurement/purchase-orders/${poId}/closure-status`)
        .set('Authorization', `Bearer ${superToken}`)
        .expect(200);
    ...
    it('closes PO atomically', async () => {
      const res = await api(app)
        .post(`/procurement/purchase-orders/${poId}/close`)
        .set('Authorization', `Bearer ${superToken}`)
...
describe('Procurement — Exception & Negative Paths', () => {
  ...
  describe('A · Authorization (403)', () => {
    ...
    it('Sales Executive cannot approve an indent (403)', async () => {
      ...
      const row = await prisma.purchaseIndent.findUnique({
        where: { id: indId },
      });
      ...
      await api(app)
        ...
        .send({
          remarks: 'Hack',
          items: [{ productId, approvedQuantity: 10, quantity: 10 }],
        })
        .expect(403);
    });
  ...
  describe('B · Plant Head Indent — return & reject', () => {
    it('Plant Head returns indent (RETURNED_TO_STORE)', async () => {
      ...
      const r1 = await api(app)
        ...
        .send({
          ...
          businessReason: 'Return test',
          items: [{ productId, quantity: 5, estimatedUnitRate: 10 }],
        })
        .expect(201);
      const inId = r1.body.data.id as string;
      cleanupIds.indents.push(inId);
      await api(app)
        .post(`/procurement/indents/${inId}/submit`)
        .set('Authorization', `Bearer ${storeToken}`)
        .send({})
        .expect(201);

      const res = await api(app)
        .post(`/procurement/indents/${inId}/return`)
        .set('Authorization', `Bearer ${plantToken}`)
        .send({ remarks: 'Need more info' })
        .expect(201);
      // Backend uses PLANT_HEAD_CORRECTION_REQUIRED for the returned state
      expect(['RETURNED_TO_STORE', 'PLANT_HEAD_CORRECTION_REQUIRED']).toContain(
    ...
    it('Plant Head rejects indent (PLANT_HEAD_REJECTED)', async () => {
      ...
      const res = await api(app)
        .post(`/procurement/indents/${inId}/reject`)
        .set('Authorization', `Bearer ${plantToken}`)
        .send({ remarks: 'Not required' })
        .expect(201);
    ...
    it('cannot approve an already-rejected indent', async () => {
      ...
      await api(app)
        ...
        .expect(201);
      await api(app)
        .post(`/procurement/indents/${inId}/approve`)
        .set('Authorization', `Bearer ${plantToken}`)
        .send({
          remarks: 'Oops',
          items: [{ productId, approvedQuantity: 5, quantity: 5 }],
        })
        .expect(409);
    });
  });
  ...
  describe('C · Super Admin PO — return & reject', () => {
    it('Super Admin returns PO (RETURNED_FOR_CORRECTION)', async () => {
      ...
      await api(app)
        .post(`/procurement/purchase-orders/${pId}/submit`)
        .set('Authorization', `Bearer ${financeToken}`)
        .send({})
        .expect(201);

      const res = await api(app)
        .post(`/procurement/purchase-orders/${pId}/return`)
        .set('Authorization', `Bearer ${superToken}`)
        .send({ remarks: 'Wrong vendor' })
        .expect(201);
      // Backend uses CORRECTION_REQUIRED for the returned state
      expect(['RETURNED_FOR_CORRECTION', 'CORRECTION_REQUIRED']).toContain(
        res.body.data.status,
      );
    });
    ...
    it('Super Admin rejects PO (SUPER_ADMIN_REJECTED)', async () => {
      ...
      const res = await api(app)
        .post(`/procurement/purchase-orders/${pId}/reject`)
        .set('Authorization', `Bearer ${superToken}`)
        .send({ remarks: 'Budget cut' })
        .expect(201);
    ...
    it('quantity mismatch flags MATCH_EXCEPTION', async () => {
      ...
      const r = await api(app)
        ...
        .send({
          supplierId,
          purchaseOrderId: pId,
          invoiceNumber: `INV-QMIS-${Date.now()}`,
          totalAmount: 1180,
          dueDate: new Date(),
          items: [
            {
      ...
      await api(app)
        ...
        .send({})
        .expect(201);
      const matchRes = await api(app)
        .post(`/procurement/vendor-invoices/${invId}/run-match`)
        .set('Authorization', `Bearer ${financeToken}`)
        .send({})
        .expect(201);
    ...
    it('rate mismatch flags MATCH_EXCEPTION', async () => {
      ...
      const r = await api(app)
        ...
        .send({
          supplierId,
          purchaseOrderId: pId,
          invoiceNumber: `INV-RMIS-${Date.now()}`,
          totalAmount: 2950,
          dueDate: new Date(),
          items: [
            {
    ...
    it('invoice with the same invoiceNumber is rejected (409)', async () => {
      ...
      const r1 = await api(app)
        ...
        .send({
          ...
          totalAmount: 236,
          dueDate: new Date(),
          items: [{ productId, quantity: 20, unitRate: 10, gstPercent: 18 }],
        })
        .expect(201);
    ...
    it('paying part of an invoice sets status to PARTIALLY_PAID', async () => {
      ...
      const invR = await api(app)
        ...
        .send({
          ...
          totalAmount: 472,
          dueDate: new Date(),
          items: [{ productId, quantity: 40, unitRate: 10, gstPercent: 18 }],
        })
        .expect(201);
      ...
      const payR = await api(app)
        ...
        .send({
          supplierId,
          paidAmount: 200,
          allocations: [{ vendorInvoiceId: invId, amount: 200 }],
        })
        .expect(201);
      ...
      await api(app)
        .post(`/procurement/vendor-payments/${payId}/submit`)
        .set('Authorization', `Bearer ${financeToken}`)
        .send({})
        .expect(201);
      await api(app)
        .post(`/procurement/vendor-payments/${payId}/approve`)
        .set('Authorization', `Bearer ${superToken}`)
        .send({})
        .expect(201);
      await api(app)
        .post(`/procurement/vendor-payments/${payId}/process`)
        .set('Authorization', `Bearer ${financeToken}`)
        .send({})
        .expect(201);
      await api(app)
        .post(`/procurement/vendor-payments/${payId}/complete`)
        .set('Authorization', `Bearer ${financeToken}`)
        .send({ transactionId: `TX-PART-${Date.now()}` })
        .expect(201);
    ...
    it('failing a payment sets status to FAILED', async () => {
      ...
      const invR = await api(app)
        ...
        .send({
          ...
          totalAmount: 177,
          dueDate: new Date(),
          items: [{ productId, quantity: 15, unitRate: 10, gstPercent: 18 }],
        })
        .expect(201);
      const invId = invR.body.data.id as string;
      cleanupIds.invoices.push(invId);
      ...
      const payR = await api(app)
        ...
        .send({
          supplierId,
          paidAmount: 177,
          allocations: [{ vendorInvoiceId: invId, amount: 177 }],
        })
        .expect(201);
      const payId = payR.body.data.id as string;
      cleanupIds.payments.push(payId);
      ...
      const res = await api(app)
        .post(`/procurement/vendor-payments/${payId}/fail`)
        .set('Authorization', `Bearer ${financeToken}`)
        .send({ reason: 'Bank rejected' })
        .expect(201);
  ...
  describe('L · Closure blocked scenarios', () => {
    it('PO with pending GRN is NOT eligible for closure', async () => {
      ...
      await api(app)
        .post(`/procurement/grns/${gId}/submit`)
        .set('Authorization', `Bearer ${storeToken}`)
        .send({})
        .expect(201);
    ...
    it('PO with unpaid invoice is NOT eligible for closure', async () => {
      ...
      const invR = await api(app)
        ...
        .send({
          supplierId,
          purchaseOrderId: pId,
          invoiceNumber: `INV-BLOCK-${Date.now()}`,
          totalAmount: 118,
          dueDate: new Date(),
          ...
          items: [
            {
              ...
              acceptedQuantity: 7,
              rejectedQuantity: 3,
              inspectionRemarks: 'Partial reject',
            },
          ],
        })
        .expect(201);
  ...
  describe('M · Audit history endpoints', () => {
    it('indent history is non-empty after actions', async () => {
      ...
      const res = await api(app)
        .get(`/procurement/indents/${inId}/history`)
        .set('Authorization', `Bearer ${superToken}`)
        .expect(200);
