  ...
  describe('1. Production Plan & Work Orders Lifecycle', () => {
    ...
    it('GET /super-admin/analytics/production returns production analytics', async () => {
      ...
      const resProgress = await request(app.getHttpServer())
        .get(
          '/super-admin/analytics/production?from=2026-07-31&to=2026-08-19&status=IN_PROGRESS',
        )
        .set('Authorization', `Bearer ${adminToken}`);
      ...
      const resQcFailed = await request(app.getHttpServer())
        .get(
          '/super-admin/analytics/production?from=2026-07-31&to=2026-08-19&status=QC_FAILED',
        )
        .set('Authorization', `Bearer ${adminToken}`);
      ...
      const resCreated = await request(app.getHttpServer())
        .get(
          '/super-admin/analytics/production?from=2026-07-31&to=2026-08-19&status=CREATED',
        )
        .set('Authorization', `Bearer ${adminToken}`);
      ...
      const resCompleted = await request(app.getHttpServer())
        .get(
          '/super-admin/analytics/production?from=2026-07-31&to=2026-08-19&status=COMPLETED',
        )
        .set('Authorization', `Bearer ${adminToken}`);
