...
describe('Finance Domain — E2E Suite', () => {
  ...
  describe('1. Finance Operations (Invoices, Payments, Ledger)', () => {
    it('lists finance invoices', async () => {
    ...
    it('lists customer ledger entries', async () => {
      const customer = await prisma.customer.findFirst();
      const res = await request(app.getHttpServer())
        .get(`/finance/ledger/${customer?.id || 'dummy-id'}`)
        .set('Authorization', `Bearer ${adminToken}`);
      expect(res.status).toBe(200);
    });
  });
