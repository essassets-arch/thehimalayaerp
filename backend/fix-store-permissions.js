...
async function main() {
  ...
  const role = await p.role.findUnique({ where: { code: 'STORE_MANAGER' } });
  if (!role) { console.log('STORE_MANAGER role not found'); return; }
  ...
  console.log('Done - assigned', assigned, 'existing permissions to STORE_MANAGER');
  await p.$disconnect();
}
