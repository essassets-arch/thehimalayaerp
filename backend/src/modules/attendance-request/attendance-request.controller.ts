...
export class AttendanceRequestController {
  ...
  approveRequest(@Param('id') id: string, @Body() body: any, @Req() req: any) {
    const userId = req.user?.sub;
    const companyId = req.headers['x-company-id'] || req.user?.companyId || 'd039cfa4-e78b-4138-adfc-1b0f14cffa91';
    return this.attendanceRequestService.approveRequest(id, userId, companyId, body);
  }
  ...
  rejectRequest(@Param('id') id: string, @Body() body: any, @Req() req: any) {
    const userId = req.user?.sub;
    const companyId = req.headers['x-company-id'] || req.user?.companyId || 'd039cfa4-e78b-4138-adfc-1b0f14cffa91';
    return this.attendanceRequestService.rejectRequest(id, userId, companyId, body);
  }
}
