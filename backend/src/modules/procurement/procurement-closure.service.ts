...
export class ProcurementClosureService {
  ...
  async evaluate(purchaseOrderId: string) {
    ...
    const received = po.grns.reduce(
      (n, g) =>
        n.add(g.items.reduce((s, i) => s.add(i.receivedQuantity), D(0))),
      D(0),
    );
    const accepted = po.grns
      ...
      .reduce(
        (n, g) =>
          n.add(g.items.reduce((s, i) => s.add(i.acceptedQuantity), D(0))),
        D(0),
      );
    const rejected = po.grns.reduce(
      (n, g) =>
        n.add(g.items.reduce((s, i) => s.add(i.rejectedQuantity), D(0))),
      D(0),
    );
    ...
    const verifiedAmount = verified.reduce(
      (n, i) => n.add(i.totalAmount),
      D(0),
    );
    const paid = active.reduce((n, i) => n.add(i.paidAmount), D(0));
    const outstanding = active.reduce(
      (n, i) => n.add(D(i.totalAmount).sub(i.paidAmount)),
      D(0),
    );
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (po.purchaseIndentId)
        await tx.purchaseIndent.updateMany({
          where: {
            id: po.purchaseIndentId,
            status: { in: ['DRAFT_PO_CREATED', 'PROCUREMENT_IN_PROGRESS'] },
          },
