const escapePdf = (value: unknown) =>
  String(value ?? '').replace(/[^\x20-\x7E]/g, '').replace(/([\\()])/g, '\\$1');
...
export function createSalarySlipPdf(slip: any): Buffer {
  const lines: Array<{ text: string; size?: number; bold?: boolean }> = [
    ...
    { text: [slip.company.email, slip.company.phone].filter(Boolean).join(' | ') },
    { text: 'SALARY SLIP', size: 18, bold: true },
    { text: `${slip.salaryMonthName} ${slip.salaryYear}`, size: 12, bold: true },
    ...
    ...slip.earnings.map((row: any) => ({ text: `${row.label}: INR ${row.amount}` })),
    { text: `Gross Earnings: INR ${slip.grossEarnings}`, bold: true },
    { text: 'DEDUCTIONS', size: 11, bold: true },
    ...slip.deductions.map((row: any) => ({ text: `${row.label}: INR ${row.amount}` })),
