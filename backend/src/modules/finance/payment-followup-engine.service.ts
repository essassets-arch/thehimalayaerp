...
@Injectable()
export class PaymentFollowupEngineService
  implements OnApplicationBootstrap, OnModuleDestroy
{
  ...
  async onApplicationBootstrap() {
    ...
    setTimeout(() => {
      this.runDailyFollowUpScan().catch((err) => {
        this.logger.error(
          `Startup daily payment scan failed: ${err?.message || err}`,
        );
      });
    }, 5000);
  ...
  private scheduleNextMidnightScan() {
    ...
    this.logger.log(
      `[Scheduler Timezone: ${timezone}] Next scheduled daily payment follow-up scan in ${Math.round(msUntilNext / 60000)} minutes`,
    );

    this.dailyTimer = setTimeout(() => {
      this.runDailyFollowUpScan().catch((err) => {
        this.logger.error(
          `Scheduled daily payment scan failed: ${err?.message || err}`,
        );
      });
  ...
  public evaluateOrderState(params: {
    ...
    const today = params.currentDate
    ...
    const termDays = this.parsePaymentTermDays(
      undefined,
      params.paymentTermDays,
    );
    const schedule = this.calculatePaymentSchedule(termDays);

    const daysElapsed = Math.max(
      0,
      Math.floor((todayMid - startMid) / 86400000),
    );
    ...
    if (outstandingAmount <= 0 && orderTotal > 0) {
      ...
      dayNumber >= schedule.reminderDay &&
      dayNumber < schedule.dueDay
    ) {
      dueState = 'DUE_SOON';
      daysRemaining = schedule.dueDay - dayNumber;
  ...
  public evaluateOrder(
    ...
    return {
      ...
      verificationStatus,
      isDelivered,
      deliveredAt: dispatchDeliveryDate
        ? new Date(dispatchDeliveryDate).toISOString()
        : null,
      ...
      lastPaymentDate: latestPayment
        ? new Date(
            latestPayment.receivedAt || latestPayment.createdAt,
          ).toISOString()
        : undefined,
      paymentReference:
        latestPayment?.paymentNo || latestPayment?.transactionReference,
    };
  }
   ...
   */
  public async runDailyFollowUpScan(
    companyId?: string,
    targetDate?: Date,
    forceScan = false,
  ) {
    ...
    if (!forceScan) {
      const alreadyExecuted = await this.prisma.auditLog
        .findFirst({
          where: {
            action: {
              in: [
                'DAILY_PAYMENT_SCAN_COMPLETED',
                'DAILY_PAYMENT_SCAN_RUNNING',
              ],
            },
      ...
      if (alreadyExecuted) {
        this.logger.log(
          `Daily payment scan for ${todayDateKey} (${scanLockKey}) already completed by instance. Skipping duplicate run.`,
        );
        return {
          success: true,
          skipped: true,
    ...
    this.logger.log(
      `Starting daily payment follow-up scan for date ${todayDateKey}...`,
    );
    ...
    for (const order of orders) {
      ...
      if (
        ...
        await this.prisma.salesOrder
          ...
          .catch((err) => {
            this.logger.warn(
              `Failed to sync sales order ${order.orderNumber}: ${err.message}`,
            );
          });
      }
