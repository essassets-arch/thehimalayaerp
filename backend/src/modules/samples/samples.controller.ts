...
export class SamplesController {
  ...
  private extractAuthData(req: any, headers: any) {
    const companyId = headers['x-company-id'] || req.user?.companyId || 'd039cfa4-e78b-4138-adfc-1b0f14cffa91';
    const userId = req.user?.sub || req.user?.id || 'a6605e65-beca-40f2-a19f-8e451e270867';
  ...
  async updateStatus(
    @Param('id') id: string,
    @Body() body: { status: SampleStatus; expectedVersion: number; extraData?: Record<string, any> },
    @Request() req,
    @Headers() headers,
  ) {
