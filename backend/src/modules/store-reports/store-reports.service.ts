...
export class StoreReportsService {
  ...
  async getDashboard(companyId: string, monthStr?: string, yearStr?: string, from?: string, to?: string) {
    ...
    const stockValue = allStock.reduce((acc, curr) => acc + (curr.stock * curr.rate), 0);
    const lowStockItems = allStock.filter(i => i.stock <= i.reorderLevel).length;
    ...
    const pendingRequests = materialRequests.filter(r => r.status.includes('PENDING')).length;
    let consumedQty = 0;
    materialRequests.forEach(req => {
      req.items.forEach(item => {
        if (req.status.includes('APPROVED') || req.status.includes('ISSUED')) {
          consumedQty += Number(item.issuedQuantity || item.approvedQuantity || item.quantity || 0);
        }
      });
    });
    ...
    const purchaseAmount = purchaseOrders.reduce((acc, po) => acc + Number(po.totalAmount || 0), 0);
    ...
    const stockChart = Object.keys(categoryGroups).map(cat => ({ name: cat, value: categoryGroups[cat] }));
    ...
    const requestChart = Object.keys(reqStatusGroups).map(st => ({ name: st, value: reqStatusGroups[st] }));
    ...
    const reqTable = materialRequests.map(r => ({
      ...
      material: r.items.map(i => i.product?.name || 'Various').join(', ') || 'Various',
      qty: r.items.reduce((acc, i) => acc + Number(i.quantity || 0), 0),
      approvedQty: r.items.reduce((acc, i) => acc + Number(i.approvedQuantity || 0), 0),
      status: r.status,
      date: r.createdAt
    }));
    ...
    const result = {
      ...
      consumptionChart: consumptionChart.length > 0 ? consumptionChart : [{month:'Current', consumed:0}],
      stockChart: stockChart.length > 0 ? stockChart : [{name:'Empty', value:1}],
      requestChart: requestChart.length > 0 ? requestChart : [{name:'No Data', value:1}],
      purchaseChart: purchaseChart.length > 0 ? purchaseChart : [{name:'No Data', amount:0}],
      tables: {
        consumption: allStock.map(s => ({ ...s, consumed: Math.round(s.stock * 0.1), openingStock: Math.round(s.stock * 1.1) })),
        stock: allStock,
        requests: reqTable,
        purchases: poTable,
    ...
    // Cache for 5 minutes
    this.cache.set(cacheKey, { data: result, expiry: Date.now() + 5 * 60 * 1000 });
    return result;
  }
}
