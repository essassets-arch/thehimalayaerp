...
async function test() {
  ...
  const rawRows = parseCSV(content).slice(1).filter(r => r.some(c => c && c.trim()));

  let matchedCount = 0;
  for (const r of rawRows) {
    const pType = (r[15] || '').trim();
    const pSize = (r[16] || '').trim();
    const pCap = (r[17] || '').trim();
    const p = findProduct(pType, pSize, pCap, products);
    if (p) matchedCount++;
  }
  console.log(`Matched ${matchedCount} / ${rawRows.length} items`);
}
