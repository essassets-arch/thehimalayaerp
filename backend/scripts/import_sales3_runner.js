...
function parseAddressObj(addrStr, stateStr, cityStr, pincodeStr) {
  ...
  if (!city) {
    ...
    else if (/AHMEDABAD|GHATLODIA|AMBAWADI|SOLA|BOPAL|THALTEJ/i.test(line1)) city = 'Ahmedabad';
...
async function main() {
  ...
  if (!csvPath) {
    ...
    for (const dir of searchDirs) {
      if (fs.existsSync(dir)) {
        const match = fs.readdirSync(dir).find(f => f.toLowerCase().includes('ravi') && f.endsWith('.csv'));
        if (match) {
  ...
  // 1. Resolve Sales 3 User
  const user = await prisma.user.findFirst({
    where: { email: { equals: 'sales3@himalayaerp.com', mode: 'insensitive' } }
  });

  if (!user) {
    console.error(`❌ User sales3@himalayaerp.com not found in database.`);
    return;
  }
  const userId = user.id;
  console.log(`Resolved Sales 3 user: ${user.name} (${user.id})`);

  // 2. Clear previous leads created by sales3 / Ravi Thakor CSV
  const cleared = await prisma.lead.deleteMany({
    where: {
      OR: [
        { createdById: userId },
        { salesExecutiveId: userId },
        { remarks: 'Imported from Ravi Thakor / Sales 3 CSV' }
      ]
    }
  });
  console.log(`Cleared ${cleared.count} existing leads for Sales 3 to ensure a clean, precise import.`);
  ...
  console.log(`Seeding ${consolidatedLeads.length} consolidated leads for Sales 3 (SA)...`);
  let successCount = 0;
  ...
  for (const gl of consolidatedLeads) {
    ...
    const cleanPhone = (gl.site_incharge_mobile || gl.office_contact || 'N/A').replace(/^MO\s*:\s*/i, '').trim();
  ...
  console.log(`✅ [SUCCESS] Imported ${successCount} leads with ${consolidatedLeads.reduce((a, b) => a + b.items.length, 0)} total line items into database.`);
