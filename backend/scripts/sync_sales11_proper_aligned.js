...
async function syncSales11Database(config) {
  console.log(`\n======================================================================`);
  console.log(`SYNCHRONIZING SALES 11: ${config.name}`);
  console.log(`Target: sales11@himalayaerp.com (Sales Executive)`);
  console.log(`======================================================================`);
  ...
  try {
    ...
    const candidatePaths = [
      ...
      path.resolve('backend/scripts/HL_data(sales3) (1).csv'),
      path.resolve('scripts/HL_data(sales3) (1).csv'),
    ...
    if (!csvPath) {
      throw new Error('Sales 11 CSV file not found in candidate paths!');
    }

    const csvContent = fs.readFileSync(csvPath, 'utf8');
    const rows = parseCSV(csvContent).slice(1).filter(r => r.length > 5 && (r[0] || r[1]) && r[1]);

    console.log(`Loaded ${rows.length} item rows from Sales 11 CSV.`);

    // 1. Identify Sales 11 User and Company
    let user = await prisma.user.findFirst({
      where: {
        email: { equals: 'sales11@himalayaerp.com', mode: 'insensitive' }
      },
    ...
    if (!user) {
      user = await prisma.user.findFirst({
        where: {
          OR: [
            { name: { equals: 'Sales Eleven', mode: 'insensitive' } },
            { name: { equals: 'Sales 11', mode: 'insensitive' } },
            { name: { equals: 'Sales Executive 11', mode: 'insensitive' } }
          ]
        },
        include: { role: true, company: true }
      });
    ...
    if (!user) {
      ...
      console.log(`Created user sales11@himalayaerp.com (${user.id})`);
    }

    const userId = user.id;
    const companyId = user.companyId || (await prisma.company.findFirst())?.id;

    console.log(`Sales 11 User: ${user.name} (${user.email}), ID: ${userId}, Company: ${companyId}`);

    // 2. Clean existing Sales 11 data strictly (NEVER TOUCH SUPERSALES 1, SUPERSALES 2, SALES 1, SALES 2, SALES 4, OR SALES 12)
    console.log('Cleaning existing Sales 11 records (All other sales data will be completely untouched)...');

    const existingOrders = await prisma.salesOrder.findMany({
      where: {
        OR: [
          { orderNumber: { gte: 'HCPPL/2627/0261', lte: 'HCPPL/2627/0263' } },
          { orderNumber: { startsWith: 'SO-S11-' } },
    ...
    const quotes = await prisma.quotation.findMany({
      where: {
        OR: [
          { quotationNumber: { gte: 'QT/2627/0261', lte: 'QT/2627/0263' } },
          { quotationNumber: { gte: 'QU/2627/0261', lte: 'QU/2627/0263' } },
          { createdById: userId },
          { salesExecutiveId: userId }
        ]
      },
    ...
    await prisma.lead.deleteMany({
      where: {
        OR: [
          { leadNumber: { gte: 'LD/2627/0261', lte: 'LD/2627/0263' } },
          { leadNumber: { gte: 'LEAD/2627/0261', lte: 'LEAD/2627/0263' } },
          { createdById: userId },
          { salesExecutiveId: userId },
          { assignedToId: userId }
        ]
    ...
    console.log('Existing Sales 11 records wiped cleanly. (All other sales data preserved intact)');
    ...
    // 4. Group Sales 11 Rows into Distinct Customer Order Groups
    const groups = [];
    ...
    console.log(`Processing ${groups.length} distinct grouped transactions for Sales 11...`);
    ...
    for (let idx = 0; idx < groups.length; idx++) {
      const g = groups[idx];
      // Sequence continues right after Sales 12 (260) -> 261 to 263
      ...
      const customerName = g.gstName || g.proj || g.grp || `Customer S11-${idx + 1}`;
      ...
      // B. Create Lead (1:1) in exact sequence LD/2627/0261...
      ...
      const createdLead = await prisma.lead.create({
        data: {
          ...
          unit: 'SET',
          remarks: 'Imported from Sales 11 CSV',
      ...
      // C. Create Quotation (1:1) in exact sequence QT/2627/0261...
      ...
      const createdQuote = await prisma.quotation.create({
        data: {
          ...
          remarks: 'Imported from Sales 11 Pipeline',
          items: {
      ...
      // D. Create Sales Order (1:1) in exact sequence HCPPL/2627/0261...
      ...
      const createdOrder = await prisma.salesOrder.create({
        data: {
          ...
          billingAddress: parsedAddr,
          shippingAddress: parsedAddr,
          remarks: 'Imported from Sales 11 CSV - Sent to Plant Head',
          version: 1,
          createdAt: leadDateObj,
          items: {
            create: groupItemsData.map(gi => ({
      ...
      // E. Send Order to Plant Head -> Create Production Plan in exact sequence PLAN/2627/0261...
      ...
      // F. Create Work Orders in exact sequence WO/2627/0542... (continuing from 541)
      for (let itemIdx = 0; itemIdx < createdOrder.items.length; itemIdx++) {
        const orderItem = createdOrder.items[itemIdx];
        const woSeqStr = String(541 + (++totalWorkOrdersCount)).padStart(4, '0');
    ...
    const nextOrderNum = 260 + groups.length + 1; // 264
    const nextWoNum = 541 + totalWorkOrdersCount + 1; // 549
    ...
    console.log(`\n======================================================================`);
    console.log(`SYNC FINISHED FOR Sales 11 (${config.name})`);
    console.log(`======================================================================`);
    console.log(`Assigned Strictly to                : ${user.name} (${user.email})`);
    console.log(`Total Leads Created (WON)           : ${groups.length} (LD/2627/0261 - LD/2627/0263)`);
    console.log(`Total Quotations Created (APPROVED) : ${groups.length} (QT/2627/0261 - QT/2627/0263)`);
    console.log(`Total Sales Orders (CONFIRMED)      : ${groups.length} (HCPPL/2627/0261 - HCPPL/2627/0263)`);
    console.log(`Total Production Plans (Plant Head) : ${groups.length} (PLAN/2627/0261 - PLAN/2627/0263)`);
    console.log(`Total Work Orders (READY FOR DISPATCH): ${totalWorkOrdersCount} (WO/2627/0542 - WO/2627/0548)`);
    console.log(`Total QC Inspections (PASSED)       : ${totalWorkOrdersCount}`);
    console.log(`======================================================================\n`);
