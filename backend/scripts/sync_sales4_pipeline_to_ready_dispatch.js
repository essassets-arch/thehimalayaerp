...
function loadSales4Leads() {
  const jsonCandidates = [
    path.resolve('backend/scripts/sales4_leads_data.json'),
    path.resolve('scripts/sales4_leads_data.json'),
    path.resolve('/app/scripts/sales4_leads_data.json'),
    path.join(__dirname, 'sales4_leads_data.json'),
  ];
  ...
  const candidatePaths = [
    path.resolve('backend/scripts/gulshan_data_sales4.csv'),
    path.resolve('scripts/gulshan_data_sales4.csv'),
    path.resolve('/app/scripts/gulshan_data_sales4.csv'),
    path.join(__dirname, 'gulshan_data_sales4.csv'),
    path.resolve('Gulshan_data(sales4).csv'),
  ];

  let csvPath = candidatePaths.find(p => fs.existsSync(p));
  if (!csvPath) {
    throw new Error(`Sales 4 CSV not found in candidate paths: ${candidatePaths.join(', ')}`);
  }

  console.log(`Reading Sales 4 CSV from: ${csvPath}`);
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    if (hasLeadInfo) {
      lastLead = {
        ...
        contactPerson: siteIncharge || 'GN',
        phone: siteInchargeMobile || officeContact || '9624659592',
        email: email || 'info@thehimalaya.co.in',
        salesRep: salesRep || 'sales4',
        address: parsedAddr,
        remarks: 'Sales 4 Lead (Converted to Quotation, Order & Ready for Dispatch)',
        source: 'OTHER',
        items: [itemObj]
      };
...
async function syncSales4PipelineToReadyDispatch(config, groups) {
  console.log(`\n======================================================================`);
  console.log(`🚀 SYNCING SALES 4 FULL PIPELINE (LEAD -> QUOTE -> ORDER -> PROD -> QC -> READY FOR DISPATCH)`);
  ...
  try {
    // 1. Resolve Sales 4 User strictly
    ...
    if (!user) {
      console.log(`Creating Sales 4 user in ${config.name}...`);
    ...
    console.log(`Resolved Sales 4 user: ${user.name} (${user.email} - ${userId})`);
    ...
    // 2. Clean ONLY previous Sales 4 orders & work orders cleanly if any exist
    console.log('Cleaning previous Sales 4 orders, quotes, plans, and work orders...');
    ...
    for (let idx = 0; idx < groups.length; idx++) {
      ...
      const contactPerson = g.contactPerson || 'GN';
      const phone = g.phone || '9624659592';
      ...
      const quotePayload = {
        ...
        remarks: 'Converted from Sales 4 Lead - Ready for Order'
      };
      ...
      const orderPayload = {
        ...
        billingAddress: parsedAddr,
        shippingAddress: parsedAddr,
        remarks: 'Sales 4 Order - Plant Head Accepted - Production & QC Passed - Ready For Dispatch',
        version: 1,
        createdAt: leadDateObj
      };
...
async function main() {
  console.log('========================================================================');
  console.log('📦 SALES 4 (GN) COMPLETE LIFECYCLE PIPELINE SYNC TO READY FOR DISPATCH');
  console.log('========================================================================');

  const groups = loadSales4Leads();
  console.log(`Total Sales 4 Orders to Process: ${groups.length}`);
  ...
  for (const db of targetDbs) {
    try {
      await syncSales4PipelineToReadyDispatch(db, groups);
    } catch (e) {
  ...
  console.log('All Sales 4 leads are WON -> Quoted -> Ordered -> Production Completed -> QC Passed -> Staged Ready for Dispatch.');
...
module.exports = { syncSales4PipelineToReadyDispatch, loadSales4Leads };
