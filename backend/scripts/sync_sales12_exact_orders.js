...
function parseAddressObj(addr, stateVal, cityVal, pincodeVal) {
  if (!addr && !cityVal && !pincodeVal) {
    return {
      line1: 'Plot No. 12, Industrial Area',
      city: 'Ahmedabad',
      state: 'Gujarat',
      country: 'India',
      pincode: '380001'
  ...
  let line1 = (addr || '').replace(/\r?\n/g, ', ').replace(/\s+/g, ' ').trim();
  let city = (cityVal || '').trim();
  let state = (stateVal || '').trim();
  let pincode = (pincodeVal || '').trim();

  if (!pincode) {
    const pinMatch = line1.match(/\b(3\d{5})\b/);
  ...
  if (!city) {
    ...
    else if (/VATVA/i.test(line1)) city = 'Ahmedabad';
    else city = 'Ahmedabad';
  }
...
async function syncSales12ExactOrders(config) {
  console.log(`\n======================================================================`);
  console.log(`SYNCHRONIZING SALES 12 (JYOTI) EXACT PIPELINE: ${config.name}`);
  console.log(`Target User: Jyoti (Sales 12) | Sales Executive | sales12@himalayaerp.com`);
  ...
  try {
    ...
    const candidatePaths = [
      path.resolve('jyoti.csv'),
      path.resolve('backend/scripts/jyoti.csv'),
      path.resolve('scripts/jyoti.csv'),
      path.join(__dirname, 'jyoti.csv')
    ];

    const csvPath = candidatePaths.find(p => fs.existsSync(p));
    if (!csvPath) {
      throw new Error('jyoti.csv not found!');
    }

    const csvContent = fs.readFileSync(csvPath, 'utf8');
    const rows = parseCSV(csvContent).slice(1).filter(r => r.length > 5 && (r[0] || r[1]));
    console.log(`Loaded ${rows.length} rows from jyoti.csv.`);

    // 1. Ensure Sales 12 User with name 'Jyoti (Sales 12)' and role 'Sales Executive'
    ...
    if (!user) {
      ...
      console.log(`Created user Jyoti (Sales 12) (${user.id})`);
    } else {
      ...
      console.log(`Updated user ${user.id} -> name: Jyoti (Sales 12), email: ${user.email}, role: ${user.role?.name}`);
    }

    const userId = user.id;
    const companyId = company?.id || user.companyId;

    // 2. Clean existing Sales 12 records (SuperSales 1 & 2, Sales 1, 2, 3, 4 preserved intact)
    console.log('Cleaning existing Sales 12 records...');
    const targetOrderNumbers = [];
    for (let i = 265; i <= 270; i++) {
    ...
    const existingOrders = await prisma.salesOrder.findMany({
      where: {
        OR: [
          ...
          { orderNumber: { gte: 'HCPPL/2627/0265', lte: 'HCPPL/2627/0270' } },
    ...
    const existingQuotes = await prisma.quotation.findMany({
      where: {
        OR: [
          ...
          { quotationNumber: { gte: 'QT/2627/0265', lte: 'QT/2627/0270' } },
          { quotationNumber: { gte: 'QU/2627/0265', lte: 'QU/2627/0270' } },
          { createdById: userId },
          { salesExecutiveId: userId }
        ]
      },
    ...
    const existingLeads = await prisma.lead.findMany({
      where: {
        OR: [
          ...
          { leadNumber: { gte: 'LD/2627/0265', lte: 'LD/2627/0270' } },
          { leadNumber: { gte: 'LEAD/2627/0265', lte: 'LEAD/2627/0270' } },
          { createdById: userId },
          { salesExecutiveId: userId },
          { assignedToId: userId }
        ]
    ...
    console.log('Existing Sales 12 records cleaned cleanly.');
    ...
    console.log(`Processing ${groups.length} distinct grouped transactions for Sales 12...`);

    // Sales 4 ended at 0264 -> Sales 12 starts at 0265
    const baseSeqOffset = 264;
    ...
    for (let idx = 0; idx < groups.length; idx++) {
      ...
      const createdLead = await prisma.lead.create({
        data: {
          ...
          gstNumber: gstinVal || customer.gstin,
          address: parsedAddr,
          source: 'OTHER',
          ...
          unit: 'SET',
          remarks: 'Imported from Sales 12 CSV (jyoti)',
      ...
      const quotationNumber = `QU/2627/${seqStr}`;
      ...
      // D. Create Sales Order (Confirmed - Stop at Orders Page)
      ...
      console.log(`  ✓ [${idx + 1}/${groups.length}] ${orderNumber} | ${customerName.padEnd(32)} | Items: ${groupItemsData.length} | ₹${grandTotal.toFixed(2)} (CONFIRMED)`);
    }

    console.log(`\n======================================================================`);
    console.log(`SUCCESS: Synced all ${groups.length} Sales 12 orders into ${config.name}!`);
    console.log(`Sequence Range: HCPPL/2627/0265 to HCPPL/2627/0270`);
    console.log(`Page: ORDERS PAGE (Confirmed - 0 Production Plans created)`);
    console.log(`All other sales representatives remain completely untouched!`);
    console.log(`======================================================================\n`);
...
async function main() {
  ...
  for (const cfg of uniqueTargetDbs) {
    try {
      await syncSales12ExactOrders(cfg);
      successCount++;
    } catch (err) {
