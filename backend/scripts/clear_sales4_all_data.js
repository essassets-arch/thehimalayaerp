...
async function clearSales4AllData(config) {
  console.log(`\n======================================================================`);
  console.log(`🧹 REMOVING ALL DATA FOR SALES FOUR (LEADS, QUOTES, ORDERS, PRODUCTION, DISPATCH)`);
  ...
  try {
    ...
    if (!user) {
      console.log(`⚠️ User sales4@himalayaerp.com not found in ${config.name}.`);
      return;
    }
    const userId = user.id;
    console.log(`Found Sales 4 user: ${user.name} (${user.id}) [${user.email}]`);

    // 0. Find all Leads for Sales 4
    const userLeads = await prisma.lead.findMany({
      where: {
        OR: [
          ...
          { assignedToId: userId },
          { remarks: { contains: 'Sales 4', mode: 'insensitive' } },
          { remarks: { contains: 'Sales Four', mode: 'insensitive' } },
          { remarks: { contains: 'MK', mode: 'insensitive' } }
        ]
      },
      select: { id: true, leadNumber: true, companyName: true }
    });
    const leadIds = userLeads.map(l => l.id);
    console.log(`Found ${userLeads.length} Leads for Sales 4 to remove.`);

    // 1. Find all Quotations for Sales 4
    ...
    // 2. Find all Sales Orders for Sales 4
    ...
    // 3. Find all Samples for Sales 4
    let sampleIds = [];
    try {
    ...
    if (orderIds.length > 0) {
      ...
      console.log('Cleaning up Dispatch records linked to Sales 4 orders...');
      ...
      console.log('Cleaning up Invoices & Payment Allocations linked to Sales 4 orders...');
    ...
    // 6. Delete Sales 4 Reminders / Daily Tasks
    try {
    ...
    // 7. Delete Lead Activities, Follow-Ups & Leads for Sales 4
    if (leadIds.length > 0) {
    ...
    // 8. Verify Remaining Counts for Sales 4
    ...
    console.log(`\n======================================================================`);
    console.log(`✅ FINAL STATUS FOR SALES FOUR (${user.email}):`);
    ...
    console.log(`   - Result                   : 100% EMPTY SLATE FOR SALES FOUR!`);
    console.log(`======================================================================\n`);
