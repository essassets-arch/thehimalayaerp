...
async function syncSuperSales2ForManualSendAndConvert(config, leadsList) {
  console.log(`\n======================================================================`);
  console.log(`🚀 SYNCING SUPERSALES 2 (LEADS & FRESH QUOTATIONS FOR MANUAL SEND/CONVERT)`);
  ...
  try {
    ...
    console.log('Cleaning existing records for fresh quotation conversion workflow...');
    ...
    for (const gl of leadsList) {
      ...
      // B. Create Quotation in Initial Status (Ready to Send & Convert in UI)
    ...
    console.log(`   - Fresh Quotations (Unconverted): ${createdCount}`);
    console.log(`   - Existing Sales Orders       : 0 (Cleared for UI manual conversion)`);
    console.log(`   - "Send Quotation" and "Convert to Order" are now fully active in UI!`);
    console.log(`======================================================================\n`);
...
async function main() {
  ...
  for (const db of targetDbs) {
    await syncSuperSales2ForManualSendAndConvert(db, leadsList);
  }
}
