...
async function syncSales13Database(config) {
  console.log(`\n======================================================================`);
  console.log(`SYNCHRONIZING SALES 13: ${config.name}`);
  console.log(`Target: sales13@himalayaerp.com (Sales Executive)`);
  console.log(`======================================================================`);
  ...
  try {
    ...
    const candidatePaths = [
      path.resolve('MK_data(sales4) (2).csv'),
      path.resolve('backend/scripts/MK_data(sales4) (2).csv'),
      path.resolve('scripts/MK_data(sales4) (2).csv'),
      path.join(__dirname, 'MK_data(sales4) (2).csv'),
      path.resolve('/app/scripts/MK_data(sales4) (2).csv'),
      path.resolve('/app/MK_data(sales4) (2).csv')
    ];

    const csvPath = candidatePaths.find(p => fs.existsSync(p));
    if (!csvPath) {
      throw new Error('Sales 13 CSV file not found in candidate paths!');
    }

    const csvContent = fs.readFileSync(csvPath, 'utf8');
    const rows = parseCSV(csvContent).slice(1).filter(r => r.length > 5 && (r[0] || r[1]) && r[1]);

    console.log(`Loaded ${rows.length} item rows from Sales 13 CSV.`);

    // 1. Identify Sales 13 User and Company
    let user = await prisma.user.findFirst({
      where: {
        email: { equals: 'sales13@himalayaerp.com', mode: 'insensitive' }
      },
    ...
    if (!user) {
      user = await prisma.user.findFirst({
        where: {
          OR: [
            { name: { equals: 'Sales Thirteen', mode: 'insensitive' } },
            { name: { equals: 'Sales 13', mode: 'insensitive' } },
            { name: { equals: 'Sales Executive 13', mode: 'insensitive' } }
          ]
        },
        include: { role: true, company: true }
      });
    ...
    if (!user) {
      ...
      console.log(`Created user sales13@himalayaerp.com (${user.id})`);
    }

    const userId = user.id;
    const companyId = user.companyId || (await prisma.company.findFirst())?.id;

    console.log(`Sales 13 User: ${user.name} (${user.email}), ID: ${userId}, Company: ${companyId}`);

    // 2. Clean existing Sales 13 data strictly (NEVER TOUCH SUPERSALES 1, SUPERSALES 2, SALES 1, SALES 2, SALES 4, SALES 12, OR SALES 11)
    console.log('Cleaning existing Sales 13 records (All other sales data will be completely untouched)...');

    const existingOrders = await prisma.salesOrder.findMany({
      where: {
        OR: [
          { orderNumber: { gte: 'HCPPL/2627/0264', lte: 'HCPPL/2627/0266' } },
          { orderNumber: { startsWith: 'SO-S13-' } },
    ...
    const quotes = await prisma.quotation.findMany({
      where: {
        OR: [
          { quotationNumber: { gte: 'QT/2627/0264', lte: 'QT/2627/0266' } },
          { quotationNumber: { gte: 'QU/2627/0264', lte: 'QU/2627/0266' } },
          { createdById: userId },
          { salesExecutiveId: userId }
        ]
      },
    ...
    await prisma.lead.deleteMany({
      where: {
        OR: [
          { leadNumber: { gte: 'LD/2627/0264', lte: 'LD/2627/0266' } },
          { leadNumber: { gte: 'LEAD/2627/0264', lte: 'LEAD/2627/0266' } },
          { createdById: userId },
          { salesExecutiveId: userId },
          { assignedToId: userId }
        ]
    ...
    console.log('Existing Sales 13 records wiped cleanly. (All other sales data preserved intact)');
    ...
    // 4. Group Sales 13 Rows into Distinct Customer Order Groups
    const groups = [];
    ...
    console.log(`Processing ${groups.length} distinct grouped transactions for Sales 13...`);
    ...
    for (let idx = 0; idx < groups.length; idx++) {
      const g = groups[idx];
      // Sequence continues right after Sales 11 (263) -> 264 to 266
      const seqStr = String(263 + idx + 1).padStart(4, '0');
      const leadDateObj = parseCsvDate(g.date);
      const parsedAddr = parseAddressObj(g.addressStr, g.stateStr, g.cityStr, g.pincodeStr);

      const customerName = g.gstName || g.proj || g.grp || `Customer S13-${idx + 1}`;
      ...
      // B. Create Lead (1:1) in exact sequence LD/2627/0264...
      ...
      const createdLead = await prisma.lead.create({
        data: {
          ...
          unit: 'SET',
          remarks: 'Imported from Sales 13 CSV',
      ...
      // C. Create Quotation (1:1) in exact sequence QT/2627/0264...
      ...
      const createdQuote = await prisma.quotation.create({
        data: {
          ...
          remarks: 'Imported from Sales 13 Pipeline',
          items: {
      ...
      // D. Create Sales Order (1:1) in exact sequence HCPPL/2627/0264...
      ...
      const createdOrder = await prisma.salesOrder.create({
        data: {
          ...
          billingAddress: parsedAddr,
          shippingAddress: parsedAddr,
          remarks: 'Imported from Sales 13 CSV - Sent to Plant Head',
          version: 1,
          createdAt: leadDateObj,
          items: {
            create: groupItemsData.map(gi => ({
      ...
      // E. Send Order to Plant Head -> Create Production Plan in exact sequence PLAN/2627/0264...
      ...
      // F. Create Work Orders in exact sequence WO/2627/0549... (continuing from 548)
      for (let itemIdx = 0; itemIdx < createdOrder.items.length; itemIdx++) {
        const orderItem = createdOrder.items[itemIdx];
        const woSeqStr = String(548 + (++totalWorkOrdersCount)).padStart(4, '0');
    ...
    const nextOrderNum = 263 + groups.length + 1; // 267
    const nextWoNum = 548 + totalWorkOrdersCount + 1; // 553
    ...
    console.log(`\n======================================================================`);
    console.log(`SYNC FINISHED FOR Sales 13 (${config.name})`);
    console.log(`======================================================================`);
    console.log(`Assigned Strictly to                : ${user.name} (${user.email})`);
    console.log(`Total Leads Created (WON)           : ${groups.length} (LD/2627/0264 - LD/2627/0266)`);
    console.log(`Total Quotations Created (APPROVED) : ${groups.length} (QT/2627/0264 - QT/2627/0266)`);
    console.log(`Total Sales Orders Created          : ${groups.length} (HCPPL/2627/0264 - HCPPL/2627/0266)`);
    console.log(`Total Production Plans Created      : ${groups.length} (PLAN/2627/0264 - PLAN/2627/0266)`);
    console.log(`Total Work Orders Created (READY)   : ${totalWorkOrdersCount} (WO/2627/0549 - WO/2627/0552)`);
    console.log(`======================================================================\n`);
...
async function main() {
  for (const config of targetDbs) {
    try {
      await syncSales13Database(config);
    } catch (e) {
