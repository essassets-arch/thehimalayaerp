...
async function syncSales1JPDatabase(config) {
  console.log(`\n======================================================================`);
  console.log(`SYNCHRONIZING SALES 1 (JP): ${config.name}`);
  console.log(`Target: sales1@himalayaerp.com (Sales Executive)`);
  console.log(`======================================================================`);
  ...
  try {
    ...
    const candidatePaths = [
      ...
      path.resolve('backend/scripts/JP_data(sales6) (1).csv'),
      path.resolve('scripts/JP_data(sales6) (1).csv'),
    ...
    if (!csvPath) {
      throw new Error('Sales 1 (JP) CSV file not found in candidates!');
    }

    const csvContent = fs.readFileSync(csvPath, 'utf8');
    const rows = parseCSV(csvContent).slice(1).filter(r => r.length > 5 && r[0]);

    console.log(`Loaded ${rows.length} item rows from Sales 1 CSV.`);
    ...
    if (!user) {
      ...
      console.log(`Created user sales1@himalayaerp.com (${user.id})`);
    }

    const userId = user.id;
    const companyId = user.companyId || (await prisma.company.findFirst())?.id;

    console.log(`Sales 1 User: ${user.name} (${user.email}), ID: ${userId}, Company: ${companyId}`);

    // 2. Clean existing Sales 1 data strictly (NEVER TOUCH SUPERSALES 1 OR SUPERSALES 2)
    console.log('Cleaning existing Sales 1 records (SuperSales 1 & 2 will be completely untouched)...');

    const existingOrders = await prisma.salesOrder.findMany({
      where: {
        OR: [
          { orderNumber: { gte: 'HCPPL/2627/0168', lte: 'HCPPL/2627/0214' } },
    ...
    const quotes = await prisma.quotation.findMany({
      where: {
        OR: [
          { quotationNumber: { gte: 'QT/2627/0168', lte: 'QT/2627/0214' } },
          { quotationNumber: { gte: 'QU/2627/0168', lte: 'QU/2627/0214' } },
          { createdById: userId },
          { salesExecutiveId: userId }
        ]
      },
    ...
    await prisma.lead.deleteMany({
      where: {
        OR: [
          { leadNumber: { gte: 'LD/2627/0168', lte: 'LD/2627/0214' } },
          { leadNumber: { gte: 'LEAD/2627/0168', lte: 'LEAD/2627/0214' } },
          { createdById: userId },
          { salesExecutiveId: userId },
          { assignedToId: userId }
        ]
    ...
    // 4. Group Sales 1 (JP) Rows into Distinct Customer Order Groups
    const groups = [];
    ...
    console.log(`Processing ${groups.length} distinct grouped transactions for Sales 1 (JP)...`);
    ...
    for (let idx = 0; idx < groups.length; idx++) {
      const g = groups[idx];
      // Sequence continues right after SuperSales 2 (167) -> 168 to 214
      ...
      // B. Create Lead (1:1) in exact sequence LD/2627/0168...
      ...
      const createdLead = await prisma.lead.create({
        data: {
          ...
          unit: 'SET',
          remarks: 'Imported from Sales 1 (JP) CSV',
      ...
      // C. Create Quotation (1:1) in exact sequence QT/2627/0168...
      ...
      const createdQuote = await prisma.quotation.create({
        data: {
          ...
          remarks: 'Imported from Sales 1 (JP) Pipeline',
          items: {
      ...
      // D. Create Sales Order (1:1) in exact sequence HCPPL/2627/0168...
      ...
      const createdOrder = await prisma.salesOrder.create({
        data: {
          ...
          billingAddress: parsedAddr,
          shippingAddress: parsedAddr,
          remarks: 'Imported from Sales 1 (JP) CSV - Sent to Plant Head',
          version: 1,
          createdAt: leadDateObj,
          items: {
            create: groupItemsData.map(gi => ({
      ...
      // E. Send Order to Plant Head -> Create Production Plan in exact sequence PLAN/2627/0168...
      ...
      // F. Create Work Orders in exact sequence WO/2627/0368... (continuing from 367)
    ...
    const nextOrderNum = 167 + groups.length + 1; // 215
    const nextWoNum = 367 + totalWorkOrdersCount + 1; // 461
    ...
    console.log(`Total Leads Created (WON)           : ${groups.length} (LD/2627/0168 - LD/2627/0214)`);
    console.log(`Total Quotations Created (APPROVED) : ${groups.length} (QT/2627/0168 - QT/2627/0214)`);
    console.log(`Total Sales Orders (CONFIRMED)      : ${groups.length} (HCPPL/2627/0168 - HCPPL/2627/0214)`);
    console.log(`Total Production Plans (Plant Head) : ${groups.length} (PLAN/2627/0168 - PLAN/2627/0214)`);
    console.log(`Total Work Orders (READY FOR DISPATCH): ${totalWorkOrdersCount} (WO/2627/0368 - WO/2627/0460)`);
...
async function main() {
  for (const cfg of targetDbs) {
    await syncSales1JPDatabase(cfg);
  }
}
