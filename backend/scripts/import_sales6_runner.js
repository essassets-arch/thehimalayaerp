...
async function main() {
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    if (hasLeadInfo) {
      lastLeadForCarry = {
        ...
        email: email,
        logged_in_sales_representive: 'sales6',
        address: address,
        state: state,
        city: city,
        pincode: pincode,
  ...
  // 1. Resolve Sales 6 User
  const user = await prisma.user.findFirst({
    where: { email: { equals: 'sales6@himalayaerp.com', mode: 'insensitive' } }
  });

  if (!user) {
    console.error(`❌ User sales6@himalayaerp.com not found in database.`);
    return;
  }
  const userId = user.id;
  console.log(`Resolved Sales 6 user: ${user.name} (${user.id})`);

  // 2. Clear previous leads created by sales6
  const cleared = await prisma.lead.deleteMany({
    where: {
      OR: [
        { createdById: userId },
        { salesExecutiveId: userId },
        { remarks: 'Imported from Trushna Sales 6 CSV' }
      ]
    }
  });
  console.log(`Cleared ${cleared.count} existing leads for Sales 6 to ensure a clean, precise import.`);
  ...
  console.log(`Seeding ${consolidatedLeads.length} consolidated leads for Sales 6...`);
  let successCount = 0;
  ...
  console.log(`✅ [SUCCESS] Imported ${successCount} leads with ${consolidatedLeads.reduce((a, b) => a + b.items.length, 0)} total line items into Sales 6.`);
