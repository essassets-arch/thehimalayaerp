...
async function syncSales12Database(config) {
  console.log(`\n======================================================================`);
  console.log(`SYNCHRONIZING SALES 12 (JYOTI): ${config.name}`);
  console.log(`Target: sales12@himalayaerp.com (Sales Executive)`);
  console.log(`======================================================================`);
  ...
  try {
    ...
    if (!csvPath) {
      throw new Error('Sales 12 CSV file not found in candidate paths!');
    }

    const csvContent = fs.readFileSync(csvPath, 'utf8');
    const rows = parseCSV(csvContent).slice(1).filter(r => r.length > 5 && (r[0] || r[1]) && r[1]);

    console.log(`Loaded ${rows.length} item rows from Sales 12 CSV.`);

    // 1. Identify Sales 12 User and Company
      ...
      user = await prisma.user.findFirst({
        where: {
          OR: [
            ...
            { name: { equals: 'Sales Twelve', mode: 'insensitive' } },
            { name: { equals: 'Sales 12', mode: 'insensitive' } },
            { name: { equals: 'Sales Executive 12', mode: 'insensitive' } }
          ]
        },
        include: { role: true, company: true }
      });
    ...
    if (!user) {
      ...
      console.log(`Created user sales12@himalayaerp.com (${user.id})`);
    }

    const userId = user.id;
    const companyId = user.companyId || (await prisma.company.findFirst())?.id;

    console.log(`Sales 12 User: ${user.name} (${user.email}), ID: ${userId}, Company: ${companyId}`);

    // 2. Clean existing Sales 12 data strictly (NEVER TOUCH SUPERSALES 1, SUPERSALES 2, SALES 1, SALES 2, OR SALES 4)
    console.log('Cleaning existing Sales 12 records (All other sales data will be completely untouched)...');

    const existingOrders = await prisma.salesOrder.findMany({
      where: {
        OR: [
          { orderNumber: { gte: 'HCPPL/2627/0255', lte: 'HCPPL/2627/0260' } },
          { orderNumber: { startsWith: 'SO-S12-' } },
    ...
    const quotes = await prisma.quotation.findMany({
      where: {
        OR: [
          { quotationNumber: { gte: 'QT/2627/0255', lte: 'QT/2627/0260' } },
          { quotationNumber: { gte: 'QU/2627/0255', lte: 'QU/2627/0260' } },
          { createdById: userId },
          { salesExecutiveId: userId }
        ]
      },
    ...
    await prisma.lead.deleteMany({
      where: {
        OR: [
          { leadNumber: { gte: 'LD/2627/0255', lte: 'LD/2627/0260' } },
          { leadNumber: { gte: 'LEAD/2627/0255', lte: 'LEAD/2627/0260' } },
          { createdById: userId },
          { salesExecutiveId: userId },
          { assignedToId: userId }
        ]
    ...
    console.log('Existing Sales 12 records wiped cleanly. (All other sales data preserved intact)');
    ...
    // 4. Group Sales 12 (Jyoti) Rows into Distinct Customer Order Groups
    const groups = [];
    ...
    console.log(`Processing ${groups.length} distinct grouped transactions for Sales 12 (Jyoti)...`);
    ...
    for (let idx = 0; idx < groups.length; idx++) {
      const g = groups[idx];
      // Sequence continues right after Sales 4 (254) -> 255 to 260
      const seqStr = String(254 + idx + 1).padStart(4, '0');
      const leadDateObj = parseCsvDate(g.date);
      const parsedAddr = parseAddressObj(g.addressStr, g.stateStr, g.cityStr, g.pincodeStr);

      const customerName = g.gstName || g.proj || g.grp || `Customer S12-${idx + 1}`;
      ...
      const createdLead = await prisma.lead.create({
        data: {
          ...
          unit: 'SET',
          remarks: 'Imported from Sales 12 (Jyoti) CSV',
      ...
      const createdQuote = await prisma.quotation.create({
        data: {
          ...
          remarks: 'Imported from Sales 12 (Jyoti) Pipeline',
          items: {
      ...
      const createdOrder = await prisma.salesOrder.create({
        data: {
          ...
          billingAddress: parsedAddr,
          shippingAddress: parsedAddr,
          remarks: 'Imported from Sales 12 (Jyoti) CSV - Sent to Plant Head',
          version: 1,
          createdAt: leadDateObj,
          items: {
            create: groupItemsData.map(gi => ({
      ...
      // E. Send Order to Plant Head -> Create Production Plan in exact sequence PLAN/2627/0255...
      ...
      // F. Create Work Orders in exact sequence WO/2627/0532... (continuing from 531)
      for (let itemIdx = 0; itemIdx < createdOrder.items.length; itemIdx++) {
        const orderItem = createdOrder.items[itemIdx];
        const woSeqStr = String(531 + (++totalWorkOrdersCount)).padStart(4, '0');
    ...
    const nextOrderNum = 254 + groups.length + 1; // 261
    const nextWoNum = 531 + totalWorkOrdersCount + 1; // 542
    ...
    console.log(`\n======================================================================`);
    console.log(`SYNC FINISHED FOR Sales 12 (Jyoti) (${config.name})`);
    console.log(`======================================================================`);
    console.log(`Assigned Strictly to                : ${user.name} (${user.email})`);
    console.log(`Total Leads Created (WON)           : ${groups.length} (LD/2627/0255 - LD/2627/0260)`);
    console.log(`Total Quotations Created (APPROVED) : ${groups.length} (QT/2627/0255 - QT/2627/0260)`);
    console.log(`Total Sales Orders (CONFIRMED)      : ${groups.length} (HCPPL/2627/0255 - HCPPL/2627/0260)`);
    console.log(`Total Production Plans (Plant Head) : ${groups.length} (PLAN/2627/0255 - PLAN/2627/0260)`);
    console.log(`Total Work Orders (READY FOR DISPATCH): ${totalWorkOrdersCount} (WO/2627/0532 - WO/2627/0541)`);
