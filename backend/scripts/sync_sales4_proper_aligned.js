...
async function syncSales4Database(config) {
  console.log(`\n======================================================================`);
  console.log(`SYNCHRONIZING SALES 4: ${config.name}`);
  console.log(`Target: sales4@himalayaerp.com (Sales Executive)`);
  console.log(`======================================================================`);
  ...
  try {
    ...
    const candidatePaths = [
      ...
      path.resolve('backend/scripts/Gulshan_data(sales5) (1).csv'),
      path.resolve('scripts/Gulshan_data(sales5) (1).csv'),
    ...
    if (!csvPath) {
      throw new Error('Sales 4 CSV file not found in candidate paths!');
    }

    const csvContent = fs.readFileSync(csvPath, 'utf8');
    const rows = parseCSV(csvContent).slice(1).filter(r => r.length > 5 && (r[0] || r[1]));

    console.log(`Loaded ${rows.length} item rows from Sales 4 CSV.`);

    // 1. Identify Sales 4 User and Company
    ...
    if (!user) {
      ...
      console.log(`Created user sales4@himalayaerp.com (${user.id})`);
    }

    const userId = user.id;
    const companyId = user.companyId || (await prisma.company.findFirst())?.id;

    console.log(`Sales 4 User: ${user.name} (${user.email}), ID: ${userId}, Company: ${companyId}`);

    // 2. Clean existing Sales 4 data strictly (NEVER TOUCH SUPERSALES 1, SUPERSALES 2, SALES 1, OR SALES 2)
    console.log('Cleaning existing Sales 4 records (All other sales data will be completely untouched)...');

    const existingOrders = await prisma.salesOrder.findMany({
      where: {
        OR: [
          { orderNumber: { gte: 'HCPPL/2627/0245', lte: 'HCPPL/2627/0254' } },
    ...
    const quotes = await prisma.quotation.findMany({
      where: {
        OR: [
          { quotationNumber: { gte: 'QT/2627/0245', lte: 'QT/2627/0254' } },
          { quotationNumber: { gte: 'QU/2627/0245', lte: 'QU/2627/0254' } },
          { createdById: userId },
          { salesExecutiveId: userId }
        ]
      },
    ...
    await prisma.lead.deleteMany({
      where: {
        OR: [
          { leadNumber: { gte: 'LD/2627/0245', lte: 'LD/2627/0254' } },
          { leadNumber: { gte: 'LEAD/2627/0245', lte: 'LEAD/2627/0254' } },
          { createdById: userId },
          { salesExecutiveId: userId },
          { assignedToId: userId }
        ]
    ...
    console.log('Existing Sales 4 records wiped cleanly. (All other sales data preserved intact)');
    ...
    // 4. Group Sales 4 Rows into Distinct Customer Order Groups
    const groups = [];
    ...
    for (let idx = 0; idx < groups.length; idx++) {
      const g = groups[idx];
      // Sequence continues right after Sales 2 (244) -> 245 to 254
      const seqStr = String(244 + idx + 1).padStart(4, '0');
      ...
      // B. Create Lead (1:1) in exact sequence LD/2627/0245...
      ...
      const createdLead = await prisma.lead.create({
        data: {
          ...
          unit: 'SET',
          remarks: 'Imported from Sales 4 CSV',
      ...
      // C. Create Quotation (1:1) in exact sequence QT/2627/0245...
      ...
      const createdQuote = await prisma.quotation.create({
        data: {
          ...
          remarks: 'Imported from Sales 4 Pipeline',
          items: {
      ...
      // D. Create Sales Order (1:1) in exact sequence HCPPL/2627/0245...
      ...
      const createdOrder = await prisma.salesOrder.create({
        data: {
          ...
          billingAddress: parsedAddr,
          shippingAddress: parsedAddr,
          remarks: 'Imported from Sales 4 CSV - Sent to Plant Head',
          version: 1,
          createdAt: leadDateObj,
          items: {
            create: groupItemsData.map(gi => ({
      ...
      // E. Send Order to Plant Head -> Create Production Plan in exact sequence PLAN/2627/0245...
      ...
      // F. Create Work Orders in exact sequence WO/2627/0515... (continuing from 514)
      for (let itemIdx = 0; itemIdx < createdOrder.items.length; itemIdx++) {
        const orderItem = createdOrder.items[itemIdx];
        const woSeqStr = String(514 + (++totalWorkOrdersCount)).padStart(4, '0');
    ...
    const nextOrderNum = 244 + groups.length + 1; // 255
    const nextWoNum = 514 + totalWorkOrdersCount + 1; // 532
    ...
    console.log(`\n======================================================================`);
    console.log(`SYNC FINISHED FOR Sales 4 (${config.name})`);
    console.log(`======================================================================`);
    console.log(`Assigned Strictly to                : ${user.name} (${user.email})`);
    console.log(`Total Leads Created (WON)           : ${groups.length} (LD/2627/0245 - LD/2627/0254)`);
    console.log(`Total Quotations Created (APPROVED) : ${groups.length} (QT/2627/0245 - QT/2627/0254)`);
    console.log(`Total Sales Orders (CONFIRMED)      : ${groups.length} (HCPPL/2627/0245 - HCPPL/2627/0254)`);
    console.log(`Total Production Plans (Plant Head) : ${groups.length} (PLAN/2627/0245 - PLAN/2627/0254)`);
    console.log(`Total Work Orders (READY FOR DISPATCH): ${totalWorkOrdersCount} (WO/2627/0515 - WO/2627/0531)`);
