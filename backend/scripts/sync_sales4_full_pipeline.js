...
async function syncSales4DataIntoDb(config, raviLeads) {
  console.log(`\n======================================================================`);
  console.log(`🚀 IMPORTING DATA INTO SALES FOUR & CONVERTING TO QUOTATIONS & ORDERS`);
  ...
  try {
    // 1. Resolve User (Sales Four / Sales Executive 4)
    ...
    if (!user) {
      console.log(`⚠️ User sales4@himalayaerp.com not found in ${config.name}. Skipping.`);
      return;
    }
    const userId = user.id;
    console.log(`Resolved Sales 4 user: ${user.name} (${user.id}) [${user.email}]`);

    // 2. Clear existing Sales 4 leads, quotes, orders
    console.log('Clearing any existing Sales 4 Leads, Quotations & Orders...');
    const sales4Orders = await prisma.salesOrder.findMany({
      where: {
        OR: [
          { createdById: userId },
    ...
    const orderIds = sales4Orders.map(o => o.id);
    ...
    const sales4Quotes = await prisma.quotation.findMany({
      where: {
        OR: [
          { createdById: userId },
          { salesExecutiveId: userId },
    ...
    const quoteIds = sales4Quotes.map(q => q.id);
...
async function main() {
  ...
  console.log(`Loaded ${raviLeads.length} consolidated leads from Sales 4 CSV.`);

  for (const db of targetDbs) {
    await syncSales4DataIntoDb(db, raviLeads);
  }
}
