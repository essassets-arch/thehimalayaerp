...
async function syncSales3ExactOrders(config) {
  console.log(`\n======================================================================`);
  console.log(`SYNCHRONIZING SALES 3 EXACT PIPELINE: ${config.name}`);
  console.log(`Target User: Sales 3 | Sales Executive | sales3@himalayaerp.com`);
  ...
  try {
    ...
    const candidatePaths = [
      path.resolve('thakor.csv'),
      path.resolve('backend/scripts/thakor.csv'),
      path.resolve('scripts/thakor.csv'),
      path.join(__dirname, 'thakor.csv')
    ];

    const csvPath = candidatePaths.find(p => fs.existsSync(p));
    if (!csvPath) {
      throw new Error('thakor.csv not found!');
    }

    const csvContent = fs.readFileSync(csvPath, 'utf8');
    const rows = parseCSV(csvContent).slice(1).filter(r => r.length > 5 && (r[0] || r[1]));
    console.log(`Loaded ${rows.length} rows from thakor.csv.`);

    // 1. Ensure Sales 3 User with exact name 'Sales 3' and role 'Sales Executive'
    ...
    if (!user) {
      ...
      console.log(`Created user Sales 3 (${user.id})`);
    } else {
      ...
      console.log(`Updated user ${user.id} -> name: Sales 3, email: ${user.email}, role: ${user.role?.name}`);
    }

    const userId = user.id;
    const companyId = user.companyId || company.id;

    // 2. Clean existing Sales 3 records if any (NEVER TOUCH SuperSales 1 & 2, Sales 1, Sales 2)
    console.log('Cleaning existing Sales 3 records (SuperSales 1 & 2, Sales 1 & 2 preserved intact)...');
    const existingOrders = await prisma.salesOrder.findMany({
      where: {
        OR: [
          { orderNumber: { gte: 'HCPPL/2627/0245', lte: 'HCPPL/2627/0300' } },
    ...
    const existingQuotes = await prisma.quotation.findMany({
      where: {
        OR: [
          { quotationNumber: { gte: 'QT/2627/0245', lte: 'QT/2627/0300' } },
          { quotationNumber: { gte: 'QU/2627/0245', lte: 'QU/2627/0300' } },
          { createdById: userId },
          { salesExecutiveId: userId }
        ]
      },
    ...
    await prisma.lead.deleteMany({
      where: {
        OR: [
          { leadNumber: { gte: 'LD/2627/0245', lte: 'LD/2627/0300' } },
          { leadNumber: { gte: 'LEAD/2627/0245', lte: 'LEAD/2627/0300' } },
          { createdById: userId },
          { salesExecutiveId: userId },
          { assignedToId: userId }
        ]
    ...
    console.log('Existing Sales 3 data cleaned cleanly.');
    ...
    // Sales 2 ended at 0244 -> Sales 3 starts at 0245
    const baseSeqOffset = 244;
    ...
    for (let idx = 0; idx < groups.length; idx++) {
      ...
      const createdLead = await prisma.lead.create({
        data: {
          ...
          unit: 'SET',
          remarks: 'Imported from Sales 3 CSV (thakor)',
    ...
    const nextOrderNum = baseSeqOffset + groups.length + 1; // 255
    ...
    console.log(`\n======================================================================`);
    console.log(`SYNC SUCCESSFUL FOR Sales 3 (${config.name})`);
    console.log(`======================================================================`);
    ...
    console.log(`Total Sales Orders Created (CONFIRMED): ${groups.length} (HCPPL/2627/0245 - HCPPL/2627/0254)`);
...
async function main() {
  ...
  for (const cfg of uniqueTargetDbs) {
    try {
      await syncSales3ExactOrders(cfg);
      successCount++;
    } catch (err) {
