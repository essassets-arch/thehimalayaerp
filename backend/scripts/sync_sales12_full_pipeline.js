...
function parseAddressObj(addrStr, stateStr, cityStr, pincodeStr) {
  ...
  if (!city) {
    if (/KHEDA/i.test(line1)) city = 'Kheda';
    else if (/SURAT/i.test(line1)) city = 'Surat';
    else if (/PETLAD|DANTALI/i.test(line1)) city = 'Petlad';
...
function loadAll7Leads() {
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    const color = obj.specidication || obj.specification || obj.color || 'GREY';
    ...
    // Note: Group rows by (lead_date + project_name + site_incharge_mobile)
    // This correctly gives 2 leads for BOOTBHAVANI (since mobile numbers are different: 8866667755 vs 84888 11670)
    // and 1 consolidated lead with 4 items for SARANG ENTERPRISE (mobile 972252245).
    ...
    if (hasLeadInfo) {
      lastLeadForCarry = {
        ...
        email: email,
        logged_in_sales_representive: 'sales12',
        address: address,
        state: state,
        city: city,
        pincode: pincode,
  ...
  console.log(`Loaded ${consolidatedLeads.length} distinct leads from CSV.`);
  return consolidatedLeads;
}

async function syncSales12IntoDb(config, all7Leads) {
  console.log(`\n===============================================================`);
  console.log(`🚀 IMPORTING SALES 12 (7 LEADS, 7 QUOTES, 7 ORDERS) INTO: ${config.name}`);
  console.log(`===============================================================`);

  const prisma = new PrismaClient({ datasources: { db: { url: config.url } } });

  try {
    // 1. Resolve or Create Sales 12 User
    ...
    if (!company) {
      console.error(`❌ No company found in ${config.name}. Skipping.`);
      return;
    }

    let salesRole = await prisma.role.findFirst({ where: { name: { equals: 'SALES', mode: 'insensitive' } } });
    if (!salesRole) {
      salesRole = await prisma.role.findFirst();
    }

    if (!user) {
      const passwordHash = await bcrypt.hash('sales12@123', 10);
      user = await prisma.user.create({
        data: {
          publicId: `USR-SALES-12-${Date.now()}`,
          email: 'sales12@himalayaerp.com',
          name: 'Jyoti Sales 12',
          password: passwordHash,
          companyId: company.id,
          roleId: salesRole ? salesRole.id : undefined,
          isActive: true
        }
      });
      console.log(`Created Sales 12 user: ${user.name} (${user.id})`);
    } else {
      console.log(`Resolved Sales 12 user: ${user.name} (${user.id})`);
    }
    const userId = user.id;
    const companyId = user.companyId || company.id;

    // 2. Clear previously imported Sales 12 records cleanly
    console.log('Clearing existing Sales 12 Leads, Quotations & Orders...');
    const sales12Orders = await prisma.salesOrder.findMany({
      where: {
        OR: [
          { createdById: userId },
    ...
    const orderIds = sales12Orders.map(o => o.id);
    ...
    const sales12Quotes = await prisma.quotation.findMany({
      where: {
        OR: [
          { createdById: userId },
          { salesExecutiveId: userId },
    ...
    const quoteIds = sales12Quotes.map(q => q.id);
    ...
    console.log(`Starting sequence counter across models: ${maxNum + 1}`);
    ...
    for (const gl of all7Leads) {
      ...
      const createdLead = await prisma.lead.create({
        data: {
          ...
          unit: 'SET',
          remarks: 'Imported from Trushna / Jyoti Sales 12 CSV',
    ...
    console.log(`✅ [SUCCESS] Synced ${createdLeads} Leads, ${createdQuotes} Quotations, and ${createdOrders} Orders for Jyoti (Sales 12) in ${config.name}!`);

  } catch (err) {
    console.error(`❌ Error syncing ${config.name}:`, err.message);
  } finally {
...
async function main() {
  const all7Leads = loadAll7Leads();
  console.log(`Found ${all7Leads.length} leads to sync into all databases.`);

  for (const db of targetDbs) {
    await syncSales12IntoDb(db, all7Leads);
  }
}
