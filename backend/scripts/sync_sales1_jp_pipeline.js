...
async function syncSales1JPDataIntoDb(config, jpLeads) {
  console.log(`\n======================================================================`);
  console.log(`🚀 IMPORTING JP DATA INTO SALES ONE & CONVERTING TO QUOTES & ORDERS`);
  ...
  try {
    // 1. Resolve User (Sales One / Sales Executive 1)
    ...
    if (!user) {
      console.log(`⚠️ User sales1@himalayaerp.com not found in ${config.name}. Skipping.`);
      return;
    }
    const userId = user.id;
    console.log(`Resolved Sales 1 user: ${user.name} (${user.id})`);

    // 2. Clear existing Sales 1 leads, quotes, orders
    console.log('Clearing any existing Sales 1 Leads, Quotations & Orders...');
    ...
    for (const gl of jpLeads) {
      ...
      const createdLead = await prisma.lead.create({
        data: {
          ...
          unit: 'SET',
          remarks: 'Imported from JP Sales CSV',
...
async function main() {
  const jpLeads = loadJPLeads();
  console.log(`Loaded ${jpLeads.length} consolidated leads from JP CSV.`);

  for (const db of targetDbs) {
    await syncSales1JPDataIntoDb(db, jpLeads);
  }
}
