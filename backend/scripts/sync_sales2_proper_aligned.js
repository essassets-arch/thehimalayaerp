...
async function syncSales2Database(config) {
  console.log(`\n======================================================================`);
  console.log(`SYNCHRONIZING SALES 2 (RUSHI SIR / RS): ${config.name}`);
  console.log(`Target: sales2@himalayaerp.com (Sales Executive)`);
  console.log(`======================================================================`);
  ...
  try {
    ...
    const candidatePaths = [
      ...
      path.resolve('backend/scripts/rushi_data(sales2) (4).csv'),
      path.resolve('scripts/rushi_data(sales2) (4).csv'),
    ...
    if (!csvPath) {
      throw new Error('Sales 2 CSV file not found in candidate paths!');
    }

    const csvContent = fs.readFileSync(csvPath, 'utf8');
    const rows = parseCSV(csvContent).slice(1).filter(r => r.length > 5 && (r[0] || r[1]));

    console.log(`Loaded ${rows.length} item rows from Sales 2 CSV.`);

    // 1. Identify Sales 2 User and Company
    ...
    if (!user) {
      ...
      console.log(`Created user sales2@himalayaerp.com (${user.id})`);
    }

    const userId = user.id;
    const companyId = user.companyId || (await prisma.company.findFirst())?.id;

    console.log(`Sales 2 User: ${user.name} (${user.email}), ID: ${userId}, Company: ${companyId}`);

    // 2. Clean existing Sales 2 data strictly (NEVER TOUCH SUPERSALES 1, SUPERSALES 2, OR SALES 1)
    console.log('Cleaning existing Sales 2 records (SuperSales 1, SuperSales 2 & Sales 1 will be completely untouched)...');
    ...
    const quotes = await prisma.quotation.findMany({
      where: {
        OR: [
          { quotationNumber: { gte: 'QT/2627/0215', lte: 'QT/2627/0244' } },
          { quotationNumber: { gte: 'QU/2627/0215', lte: 'QU/2627/0244' } },
          { createdById: userId },
          { salesExecutiveId: userId }
        ]
      },
    ...
    await prisma.lead.deleteMany({
      where: {
        OR: [
          { leadNumber: { gte: 'LD/2627/0215', lte: 'LD/2627/0244' } },
          { leadNumber: { gte: 'LEAD/2627/0215', lte: 'LEAD/2627/0244' } },
          { createdById: userId },
          { salesExecutiveId: userId },
          { assignedToId: userId }
        ]
    ...
    console.log('Existing Sales 2 records wiped cleanly. (SuperSales 1, SuperSales 2 & Sales 1 preserved intact)');
    ...
    // 4. Group Sales 2 (Rushi Sir) Rows into Distinct Customer Order Groups
    const groups = [];
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!date) date = '01-07-2026'; // fallback for missing date
    ...
    console.log(`Processing ${groups.length} distinct grouped transactions for Sales 2 (Rushi Sir)...`);
    ...
    for (let idx = 0; idx < groups.length; idx++) {
      const g = groups[idx];
      // Sequence continues right after Sales 1 (214) -> 215 to 244
      const seqStr = String(214 + idx + 1).padStart(4, '0');
      ...
      // B. Create Lead (1:1) in exact sequence LD/2627/0215...
      ...
      const createdLead = await prisma.lead.create({
        data: {
          ...
          unit: 'SET',
          remarks: 'Imported from Sales 2 (Rushi Sir) CSV',
      ...
      // C. Create Quotation (1:1) in exact sequence QT/2627/0215...
      ...
      const createdQuote = await prisma.quotation.create({
        data: {
          ...
          remarks: 'Imported from Sales 2 (Rushi Sir) Pipeline',
          items: {
      ...
      // D. Create Sales Order (1:1) in exact sequence HCPPL/2627/0215...
      ...
      const createdOrder = await prisma.salesOrder.create({
        data: {
          ...
          billingAddress: parsedAddr,
          shippingAddress: parsedAddr,
          remarks: 'Imported from Sales 2 (Rushi Sir) CSV - Sent to Plant Head',
          version: 1,
          createdAt: leadDateObj,
          items: {
            create: groupItemsData.map(gi => ({
      ...
      // E. Send Order to Plant Head -> Create Production Plan in exact sequence PLAN/2627/0215...
      ...
      // F. Create Work Orders in exact sequence WO/2627/0461... (continuing from 460)
      for (let itemIdx = 0; itemIdx < createdOrder.items.length; itemIdx++) {
        const orderItem = createdOrder.items[itemIdx];
        const woSeqStr = String(460 + (++totalWorkOrdersCount)).padStart(4, '0');
    ...
    const nextOrderNum = 214 + groups.length + 1; // 245
    const nextWoNum = 460 + totalWorkOrdersCount + 1; // 515
    ...
    console.log(`\n======================================================================`);
    console.log(`SYNC FINISHED FOR Sales 2 (${config.name})`);
    console.log(`======================================================================`);
    console.log(`Assigned Strictly to                : ${user.name} (${user.email})`);
    console.log(`Total Leads Created (WON)           : ${groups.length} (LD/2627/0215 - LD/2627/0244)`);
    console.log(`Total Quotations Created (APPROVED) : ${groups.length} (QT/2627/0215 - QT/2627/0244)`);
    console.log(`Total Sales Orders (CONFIRMED)      : ${groups.length} (HCPPL/2627/0215 - HCPPL/2627/0244)`);
    console.log(`Total Production Plans (Plant Head) : ${groups.length} (PLAN/2627/0215 - PLAN/2627/0244)`);
    console.log(`Total Work Orders (READY FOR DISPATCH): ${totalWorkOrdersCount} (WO/2627/0461 - WO/2627/0514)`);
