...
async function wipeSales1FromDb(config) {
  console.log(`\n======================================================================`);
  console.log(` 🗑️  COMPREHENSIVE WIPE OF SALES 1 DATA FROM: ${config.name}`);
  ...
  try {
    ...
    const user = await prisma.user.findFirst({
      where: {
        OR: [
          ...
          { name: { contains: 'Sales One', mode: 'insensitive' } },
          { name: { contains: 'Sales Executive 1', mode: 'insensitive' } }
        ]
      }
    });

    if (!user) {
      console.log(`❌ User sales1@himalayaerp.com not found in ${config.name}.`);
      return;
    }
    ...
    const allSales1Users = await prisma.user.findMany({
      where: {
        OR: [
          { email: { in: ['sales1@himalayaerp.com'] } },
          { name: { contains: 'Sales 1', mode: 'insensitive' } },
          { name: { contains: 'Sales One', mode: 'insensitive' } },
          { name: { contains: 'Sales Executive 1', mode: 'insensitive' } }
        ]
    ...
    const allUserIds = allSales1Users.map(u => u.id);
    ...
    // 2. Identify all Leads related to Sales 1
    let leadWhereClauses = [];
    if (leadCols.has('createdById')) leadWhereClauses.push(`"createdById" IN ('${allUserIds.join("','")}')`);
    if (leadCols.has('salesExecutiveId')) leadWhereClauses.push(`"salesExecutiveId" IN ('${allUserIds.join("','")}')`);
    if (leadCols.has('assignedToId')) leadWhereClauses.push(`"assignedToId" IN ('${allUserIds.join("','")}')`);
    if (leadCols.has('salespersonId')) leadWhereClauses.push(`"salespersonId" IN ('${allUserIds.join("','")}')`);
    if (leadCols.has('remarks')) {
      leadWhereClauses.push(`("remarks" ILIKE '%Sales 1%' AND "remarks" NOT ILIKE '%Super%Sales 1%' AND "remarks" NOT ILIKE '%SuperSales%')`);
      leadWhereClauses.push(`("remarks" ILIKE '%Sales One%' AND "remarks" NOT ILIKE '%Super%Sales%')`);
      leadWhereClauses.push(`"remarks" ILIKE '%Trushna%'`);
    }
    ...
    // 3. Identify all Quotations related to Sales 1
    let quoteWhereClauses = [];
    if (quoteCols.has('createdById')) quoteWhereClauses.push(`"createdById" IN ('${allUserIds.join("','")}')`);
    if (quoteCols.has('salesExecutiveId')) quoteWhereClauses.push(`"salesExecutiveId" IN ('${allUserIds.join("','")}')`);
    ...
    if (quoteCols.has('remarks')) {
      quoteWhereClauses.push(`("remarks" ILIKE '%Sales 1%' AND "remarks" NOT ILIKE '%Super%Sales 1%' AND "remarks" NOT ILIKE '%SuperSales%')`);
      quoteWhereClauses.push(`("remarks" ILIKE '%Sales One%' AND "remarks" NOT ILIKE '%Super%Sales%')`);
      quoteWhereClauses.push(`"remarks" ILIKE '%Trushna%'`);
    }
    ...
    // 4. Identify all Sales Orders related to Sales 1
    let orderWhereClauses = [];
    if (orderCols.has('createdById')) orderWhereClauses.push(`"createdById" IN ('${allUserIds.join("','")}')`);
    if (orderCols.has('salesExecutiveId')) orderWhereClauses.push(`"salesExecutiveId" IN ('${allUserIds.join("','")}')`);
    ...
    if (orderCols.has('remarks')) {
      orderWhereClauses.push(`("remarks" ILIKE '%Sales 1%' AND "remarks" NOT ILIKE '%Super%Sales 1%' AND "remarks" NOT ILIKE '%SuperSales%')`);
      orderWhereClauses.push(`("remarks" ILIKE '%Sales One%' AND "remarks" NOT ILIKE '%Super%Sales%')`);
      orderWhereClauses.push(`"remarks" ILIKE '%Trushna%'`);
    }
    ...
    // 8. Identify Customers created by Sales 1
    let custIds = [];
    try {
    ...
    // --- STEP G: DELETE CUSTOMERS CREATED BY SALES 1 ---
    if (custIds.length > 0) {
      try {
        ...
        console.log(`  ✓ Cleaned up customers created by Sales 1.`);
      } catch (e) {
    ...
    console.log(`\n🎉 [${config.name}] ALL SALES 1 DATA AND ITS COMPLETE PIPELINE HAVE BEEN PERMANENTLY REMOVED!`);
  } catch (err) {
