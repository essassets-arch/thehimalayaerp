...
async function main() {
  ...
  if (!csvPath) {
    ...
    for (const dir of searchDirs) {
      if (fs.existsSync(dir)) {
        const match = fs.readdirSync(dir).find(f => f.toLowerCase().includes('trushna') && f.endsWith('.csv'));
        if (match) {
  ...
  // 1. Resolve Sales 12 User
  const user = await prisma.user.findFirst({
    where: { email: { equals: 'sales12@himalayaerp.com', mode: 'insensitive' } }
  });

  if (!user) {
    console.error(`❌ User sales12@himalayaerp.com not found in database.`);
    return;
  }
  const userId = user.id;
  console.log(`Resolved Sales 12 user: ${user.name} (${user.id})`);

  // 2. Clear previous leads created by sales12 / Trushna CSV
  ...
  console.log(`Cleared ${cleared.count} existing leads for Sales 12 to ensure a clean, precise import.`);
  ...
  console.log(`Seeding ${consolidatedLeads.length} consolidated leads for Sales 12 (Jyoti)...`);
  let successCount = 0;
