...
const csvPath = path.resolve(__dirname, 'rushi_data(sales2) (4).csv');
...
consolidatedLeads.forEach((l, idx) => {
  console.log(`Lead #${String(idx+1).padStart(2, '0')}: Date=${l.lead_date || 'N/A'}, Company=${l.project_name}, Items=${l.items.length}, GrandTotal=₹${l.items.reduce((s, it) => s + it.grand_total, 0)}`);
});
