...
function parseAddressObj(addrStr, stateStr, cityStr, pincodeStr) {
  ...
  return {
    ...
    city: city || 'Gandhinagar',
    state: state || 'Gujarat',
    country: 'India',
    pincode: pincode || '382007'
  };
}
...
async function main() {
  ...
  if (!csvPath) {
    ...
    for (const dir of searchDirs) {
      if (fs.existsSync(dir)) {
        const match = fs.readdirSync(dir).find(f => f.toLowerCase().includes('jp') && f.endsWith('.csv'));
        if (match) {
  ...
  if (!user) {
    console.error(`❌ User sales1@himalayaerp.com not found in database.`);
    return;
  }
  ...
  console.log(`Seeding ${consolidatedLeads.length} consolidated leads for Sales 1...`);
  let successCount = 0;
  ...
  console.log(`✅ [SUCCESS] Imported ${successCount} leads with ${consolidatedLeads.reduce((a, b) => a + b.items.length, 0)} total line items into Sales 1.`);
