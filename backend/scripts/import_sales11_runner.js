...
async function main() {
  ...
  if (!user) {
    console.error(`❌ User sales11@himalayaerp.com not found in database.`);
    return;
  }
  ...
  console.log(`✅ [SUCCESS] Imported ${successCount} leads with ${consolidatedLeads.reduce((a, b) => a + b.items.length, 0)} total line items into Sales 11.`);
