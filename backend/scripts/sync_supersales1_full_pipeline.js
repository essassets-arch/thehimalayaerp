...
async function syncPipelineForDb(config, consolidatedLeads) {
  console.log(`\n======================================================================`);
  console.log(` SYNCING PIPELINE (LEADS, QUOTATIONS, ORDERS) INTO: ${config.name}`);
  ...
  try {
    ...
    // 3. Clear existing SuperSales 1 Leads, Quotations, Orders
    console.log('Clearing existing SuperSales 1 Sales Orders...');
    ...
    // Leads: LEAD/2627/0001 - LEAD/2627/0144
    // Quotations: QU/2627/0001 - QU/2627/0144
    // Sales Orders: HCPPL/2627/0001 - HCPPL/2627/0144
    let leadSeq = 1;
    let quoteSeq = 1;
    let orderSeq = 1;

    console.log(`\nImporting ${consolidatedLeads.length} leads with full quotation and order pipeline...`);
    ...
    for (const gl of consolidatedLeads) {
      ...
      let quotationNumber = `QU/${fy}/${String(quoteSeq++).padStart(4, '0')}`;
      while (await prisma.quotation.findFirst({ where: { quotationNumber }, select: { id: true } })) {
        quotationNumber = `QU/${fy}/${String(quoteSeq++).padStart(4, '0')}`;
      }

      let orderNumber = `HCPPL/${fy}/${String(orderSeq++).padStart(4, '0')}`;
      while (await prisma.salesOrder.findFirst({ where: { orderNumber }, select: { id: true } })) {
        orderNumber = `HCPPL/${fy}/${String(orderSeq++).padStart(4, '0')}`;
      }
    ...
    // Update Sequences for next creation
    ...
    await prisma.idSequence.upsert({
      ...
      update: { nextValue: quoteSeq },
      create: { key: 'quotation_number_2627', nextValue: quoteSeq }
    });
    await prisma.idSequence.upsert({
      where: { key: 'sales_order_number_2627' },
      update: { nextValue: orderSeq },
      create: { key: 'sales_order_number_2627', nextValue: orderSeq }
    });

    console.log(`✅ [${config.name}] Successfully synced:`);
    console.log(`   - ${createdLeads} Leads (${'LEAD/2627/0001'} - ${'LEAD/2627/' + String(createdLeads).padStart(4, '0')})`);
    console.log(`   - ${createdQuotes} Quotations (${'QU/2627/0001'} - ${'QU/2627/' + String(createdQuotes).padStart(4, '0')})`);
    console.log(`   - ${createdOrders} Sales Orders (${'HCPPL/2627/0001'} - ${'HCPPL/2627/' + String(createdOrders).padStart(4, '0')})`);
...
async function main() {
  ...
  for (const db of targetDbs) {
    await syncPipelineForDb(db, consolidatedLeads);
  }
}
