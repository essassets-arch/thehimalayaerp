...
async function syncSales3DataIntoDb(config, raviLeads) {
  console.log(`\n======================================================================`);
  console.log(`🚀 IMPORTING DATA INTO SALES THREE & CONVERTING TO QUOTATIONS & ORDERS`);
  ...
  try {
    // 1. Resolve User (Sales Three / Sales Executive 3)
    ...
    if (!user) {
      console.log(`⚠️ User sales3@himalayaerp.com not found in ${config.name}. Skipping.`);
      return;
    }
    const userId = user.id;
    console.log(`Resolved Sales 3 user: ${user.name} (${user.id}) [${user.email}]`);

    // 2. Clear existing Sales 3 leads, quotes, orders
    console.log('Clearing any existing Sales 3 Leads, Quotations & Orders...');
    const sales3Orders = await prisma.salesOrder.findMany({
      where: {
        OR: [
          { createdById: userId },
    ...
    const orderIds = sales3Orders.map(o => o.id);
    ...
    const sales3Quotes = await prisma.quotation.findMany({
      where: {
        OR: [
          { createdById: userId },
          { salesExecutiveId: userId },
    ...
    const quoteIds = sales3Quotes.map(q => q.id);
...
async function main() {
  const raviLeads = loadRaviLeads();
  console.log(`Loaded ${raviLeads.length} consolidated leads from Sales 3 CSV.`);

  for (const db of targetDbs) {
    await syncSales3DataIntoDb(db, raviLeads);
  }
}
