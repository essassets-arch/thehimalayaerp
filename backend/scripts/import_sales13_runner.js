...
function parseAddressObj(addrStr, stateStr, cityStr, pincodeStr) {
  ...
  if (!city) {
    if (/AHMEDABAD|PALDI|SARKHEJ/i.test(line1)) city = 'Ahmedabad';
...
async function main() {
  ...
  if (!csvPath) {
    ...
    for (const dir of searchDirs) {
      if (fs.existsSync(dir)) {
        const match = fs.readdirSync(dir).find(f => f.toLowerCase().includes('mk') && f.endsWith('.csv'));
        if (match) {
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    const color = obj.specification || obj.specifdication || obj.color || 'GREY';
    ...
    if (hasLeadInfo) {
      lastLeadForCarry = {
        ...
        email: email,
        logged_in_sales_representive: 'sales13',
        address: address,
        state: state,
        city: city,
        pincode: pincode,
  ...
  // 1. Resolve Sales 13 User
  const user = await prisma.user.findFirst({
    where: { email: { equals: 'sales13@himalayaerp.com', mode: 'insensitive' } }
  });

  if (!user) {
    console.error(`❌ User sales13@himalayaerp.com not found in database.`);
    return;
  }
  const userId = user.id;
  console.log(`Resolved Sales 13 user: ${user.name} (${user.id})`);

  // 2. Clear previous leads created by sales13
  const cleared = await prisma.lead.deleteMany({
    where: {
      OR: [
        { createdById: userId },
        { salesExecutiveId: userId },
        { remarks: 'Imported from MK Sales 13 CSV' }
      ]
    }
  });
  console.log(`Cleared ${cleared.count} existing leads for Sales 13 to ensure a clean, precise import.`);
  ...
  console.log(`Seeding ${consolidatedLeads.length} consolidated leads for Sales 13...`);
  let successCount = 0;
  ...
  console.log(`✅ [SUCCESS] Imported ${successCount} leads with ${consolidatedLeads.reduce((a, b) => a + b.items.length, 0)} total line items into Sales 13.`);
