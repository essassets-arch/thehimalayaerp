...
function loadSales3Leads() {
  const jsonCandidates = [
    path.resolve('backend/scripts/sales3_leads_data.json'),
    path.resolve('scripts/sales3_leads_data.json'),
    path.resolve('/app/scripts/sales3_leads_data.json'),
    path.join(__dirname, 'sales3_leads_data.json'),
  ];
  ...
  const candidatePaths = [
    path.resolve('backend/scripts/ravi_thakor_sales3.csv'),
    path.resolve('scripts/ravi_thakor_sales3.csv'),
    path.resolve('/app/scripts/ravi_thakor_sales3.csv'),
    path.join(__dirname, 'ravi_thakor_sales3.csv'),
    path.resolve('ravi_thakor(sales3).csv'),
  ];

  let csvPath = candidatePaths.find(p => fs.existsSync(p));
  if (!csvPath) {
    throw new Error(`Sales 3 CSV not found in candidate paths: ${candidatePaths.join(', ')}`);
  }

  console.log(`Reading Sales 3 CSV from: ${csvPath}`);
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    if (hasLeadInfo) {
      lastLead = {
        ...
        contactPerson: siteIncharge || 'RT',
        phone: siteInchargeMobile || officeContact || '9426084123',
        email: email || 'info@thehimalaya.co.in',
        salesRep: salesRep || 'sales3',
        address: parsedAddr,
        remarks: 'Sales 3 Lead (Converted to Quotation, Order & Ready for Dispatch)',
        source: 'OTHER',
        items: [itemObj]
      };
...
async function syncSales3PipelineToReadyDispatch(config, groups) {
  console.log(`\n======================================================================`);
  console.log(`🚀 SYNCING SALES 3 FULL PIPELINE (LEAD -> QUOTE -> ORDER -> PROD -> QC -> READY FOR DISPATCH)`);
  ...
  try {
    // 1. Resolve Sales 3 User strictly
    ...
    if (!user) {
      console.log(`Creating Sales 3 user in ${config.name}...`);
    ...
    console.log(`Resolved Sales 3 user: ${user.name} (${user.email} - ${userId})`);
    ...
    // 2. Clean ONLY previous Sales 3 orders & work orders cleanly if any exist
    console.log('Cleaning previous Sales 3 orders, quotes, plans, and work orders...');
    ...
    for (let idx = 0; idx < groups.length; idx++) {
      ...
      const contactPerson = g.contactPerson || 'RT';
      const phone = g.phone || '9426084123';
      ...
      const quotePayload = {
        ...
        remarks: 'Converted from Sales 3 Lead - Ready for Order'
      };
      ...
      const orderPayload = {
        ...
        billingAddress: parsedAddr,
        shippingAddress: parsedAddr,
        remarks: 'Sales 3 Order - Plant Head Accepted - Production & QC Passed - Ready For Dispatch',
        version: 1,
        createdAt: leadDateObj
      };
...
async function main() {
  console.log('========================================================================');
  console.log('📦 SALES 3 (RT) COMPLETE LIFECYCLE PIPELINE SYNC TO READY FOR DISPATCH');
  console.log('========================================================================');

  const groups = loadSales3Leads();
  console.log(`Total Sales 3 Orders to Process: ${groups.length}`);
  ...
  for (const db of targetDbs) {
    try {
      await syncSales3PipelineToReadyDispatch(db, groups);
    } catch (e) {
  ...
  console.log('All Sales 3 leads are WON -> Quoted -> Ordered -> Production Completed -> QC Passed -> Staged Ready for Dispatch.');
...
module.exports = { syncSales3PipelineToReadyDispatch, loadSales3Leads };
