...
async function clearSales3AllData(config) {
  console.log(`\n======================================================================`);
  console.log(`🧹 REMOVING ALL DATA FOR SALES THREE (LEADS, QUOTES, ORDERS, PRODUCTION, DISPATCH)`);
  ...
  try {
    const user = await prisma.user.findFirst({
      where: {
        OR: [
          ...
          { name: { contains: 'Harshil', mode: 'insensitive' } },
          { name: { contains: 'HL', mode: 'insensitive' } }
        ]
      }
    });

    if (!user) {
      console.log(`⚠️ User sales3@himalayaerp.com not found in ${config.name}.`);
      return;
    }
    const userId = user.id;
    console.log(`Found Sales 3 user: ${user.name} (${user.id}) [${user.email}]`);

    // 0. Find all Leads for Sales 3
    const userLeads = await prisma.lead.findMany({
      where: {
        OR: [
          ...
          { assignedToId: userId },
          { remarks: { contains: 'Sales 3', mode: 'insensitive' } },
          { remarks: { contains: 'Sales Three', mode: 'insensitive' } },
          { remarks: { contains: 'Harshil', mode: 'insensitive' } }
        ]
      },
      select: { id: true, leadNumber: true, companyName: true }
    });
    const leadIds = userLeads.map(l => l.id);
    console.log(`Found ${userLeads.length} Leads for Sales 3 to remove.`);

    // 1. Find all Quotations for Sales 3
    ...
    // 2. Find all Sales Orders for Sales 3
    ...
    // 3. Find all Samples for Sales 3
    let sampleIds = [];
    try {
    ...
    if (orderIds.length > 0) {
      ...
      console.log('Cleaning up Dispatch records linked to Sales 3 orders...');
      ...
      console.log('Cleaning up Invoices & Payment Allocations linked to Sales 3 orders...');
    ...
    // 6. Delete Sales 3 Reminders / Daily Tasks
    try {
    ...
    // 7. Delete Lead Activities, Follow-Ups & Leads for Sales 3
    if (leadIds.length > 0) {
    ...
    // 8. Verify Remaining Counts for Sales 3
    ...
    console.log(`\n======================================================================`);
    console.log(`✅ FINAL STATUS FOR SALES THREE (${user.email}):`);
    ...
    console.log(`   - Result                   : 100% EMPTY SLATE FOR SALES THREE!`);
    console.log(`======================================================================\n`);
