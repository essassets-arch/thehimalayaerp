...
async function syncSales1ExactOrders(config) {
  console.log(`\n======================================================================`);
  console.log(`SYNCHRONIZING SALES 1 EXACT PIPELINE: ${config.name}`);
  console.log(`Target User: Sales 1 | Sales Executive | sales1@himalayaerp.com`);
  ...
  try {
    ...
    const candidatePaths = [
      ...
      path.join(__dirname, 'jesal.csv')
    ];

    const csvPath = candidatePaths.find(p => fs.existsSync(p));
    if (!csvPath) {
      throw new Error('jesal.csv not found!');
    }
    ...
    // 1. Ensure Sales 1 User with exact name 'Sales 1' and role 'Sales Executive'
    let user = await prisma.user.findFirst({
      where: {
        OR: [
          { email: { equals: 'sales1@himalayaerp.com', mode: 'insensitive' } },
          { email: { equals: 'sales1@himalayaerp.co', mode: 'insensitive' } }
        ]
      },
    ...
    if (!user) {
      ...
      console.log(`Created user Sales 1 (${user.id})`);
    } else {
      // Update name to 'Sales 1' and ensure role is Sales Executive
      ...
      console.log(`Updated user ${user.id} -> name: Sales 1, email: ${user.email}, role: ${user.role?.name}`);
    }

    const userId = user.id;
    const companyId = user.companyId || company.id;

    // 2. Clean existing Sales 1 records if any (NEVER TOUCH SuperSales 1 & 2)
    console.log('Cleaning existing Sales 1 records (SuperSales 1 & 2 preserved intact)...');
    const existingOrders = await prisma.salesOrder.findMany({
      where: {
        OR: [
          { orderNumber: { gte: 'HCPPL/2627/0169', lte: 'HCPPL/2627/0250' } },
    ...
    const existingQuotes = await prisma.quotation.findMany({
      where: {
        OR: [
          { quotationNumber: { gte: 'QT/2627/0169', lte: 'QT/2627/0250' } },
          { quotationNumber: { gte: 'QU/2627/0169', lte: 'QU/2627/0250' } },
          { createdById: userId },
          { salesExecutiveId: userId }
        ]
      },
    ...
    await prisma.lead.deleteMany({
      where: {
        OR: [
          { leadNumber: { gte: 'LD/2627/0169', lte: 'LD/2627/0250' } },
          { leadNumber: { gte: 'LEAD/2627/0169', lte: 'LEAD/2627/0250' } },
          { createdById: userId },
          { salesExecutiveId: userId },
          { assignedToId: userId }
        ]
    ...
    console.log('Existing Sales 1 data cleaned cleanly.');
    ...
    // SuperSales 2 ended at 0168 -> Sales 1 starts at 0169
    ...
    for (let idx = 0; idx < groups.length; idx++) {
      ...
      const createdLead = await prisma.lead.create({
        data: {
          ...
          unit: 'SET',
          remarks: 'Imported from Sales 1 CSV',
    ...
    const nextOrderNum = baseSeqOffset + groups.length + 1; // 216
    ...
    console.log(`\n======================================================================`);
    console.log(`SYNC SUCCESSFUL FOR Sales 1 (${config.name})`);
    console.log(`======================================================================`);
    console.log(`User Assigned                         : ${user.name} (${user.email})`);
    console.log(`Role                                  : ${user.role?.name}`);
    console.log(`Total Leads Created (WON)             : ${groups.length} (LD/2627/0169 - LD/2627/0215)`);
    console.log(`Total Quotations Created (APPROVED)   : ${groups.length} (QT/2627/0169 - QT/2627/0215)`);
    console.log(`Total Sales Orders Created (CONFIRMED): ${groups.length} (HCPPL/2627/0169 - HCPPL/2627/0215)`);
